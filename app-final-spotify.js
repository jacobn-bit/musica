...
const publicDataDb=configured&&window.supabase&&typeof window.supabase.createClient==="function"
  ?window.supabase.createClient(SUPABASE_URL.trim(),SUPABASE_ANON_KEY.trim(),{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false,storageKey:"muze-public-catalog"}})
  :null;
...
const ALBUM_CATALOGUE_VIEW_KEY="muze-album-view";
function savedAlbumCatalogueView(){
  try{
    const value=localStorage.getItem(ALBUM_CATALOGUE_VIEW_KEY);
...
function authErrorMessage(error){
  ...
  if(/invalid api key|api key/i.test(message))return "Muze is temporarily unavailable. Please try again.";
...
async function initAuth(){
  ...
  syncAuthUi();
  if(state.view==="libraries"){
    loadLibraries().catch(error=>console.warn("Unable to refresh libraries after auth init",error));
  }else if(state.view==="chat"){
    await Promise.all([loadLibraries(),loadChatMessages(),loadUserPresence()]);render();
  }
  if(returnError){
...
function albumIsGreatestHits(album){
  ...
  const genre=genreKey(album?.genre||album?.cached_genre||album?.stored_genre||"");
  return genre==="greatest hits"||/\b(greatest hits|best of|very best of)\b/.test(title);
}
...
const albumSearchTextCache=new WeakMap();
const albumGenreLabelCache=new WeakMap();
const albumSearchTokenCache=new WeakMap();
function cachedAlbumGenreLabel(album){
  if(!album)return "Album";
  const cached=albumGenreLabelCache.get(album);
  if(cached)return cached;
  const label=albumGenreLabel(album);
  albumGenreLabelCache.set(album,label);
  return label;
}
function albumSearchText(album){
  if(!album)return "";
  const cached=albumSearchTextCache.get(album);
  if(cached)return cached;
  const text=`${album.title||""} ${album.artist||""} ${album.year||""} ${genreSearchText(album)}`.toLowerCase();
  albumSearchTextCache.set(album,text);
  return text;
}
function fuzzySearchKey(value){return String(value||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g," ").replace(/\s+/g," ").trim()}
function editDistanceWithin(a,b,limit=2){
  a=fuzzySearchKey(a);b=fuzzySearchKey(b);
  if(!a||!b)return false;
  if(Math.abs(a.length-b.length)>limit)return false;
  let previous=Array.from({length:b.length+1},(_,i)=>i);
  for(let i=1;i<=a.length;i++){
    let current=[i],rowMin=current[0];
...
function albumSearchTokens(album){
  const cached=albumSearchTokenCache.get(album);
  if(cached)return cached;
  const tokens=[album.title,album.artist,cachedAlbumGenreLabel(album),...(Array.isArray(album.genres)?album.genres:[])].flatMap(value=>fuzzySearchKey(value).split(" ")).filter(token=>token.length>=3);
  albumSearchTokenCache.set(album,tokens);
  return tokens;
}
function albumMatchesSearch(album,query){
  const q=fuzzySearchKey(query);
  if(!q)return true;
  const haystack=albumSearchText(album);
  if(haystack.includes(q))return true;
  const qTokens=q.split(" ").filter(Boolean);
  return qTokens.every(token=>albumSearchTokens(album).some(candidate=>candidate.includes(token)||token.includes(candidate)||editDistanceWithin(token,candidate,token.length<=5?1:2)));
}
...
function overviewFieldHasValue(value){
  if(value===undefined||value===null||value==="")return false;
  if(Array.isArray(value))return value.length>0;
  if(typeof value==="object")return Object.keys(value).length>0;
  return true;
}
function mergeDuplicateOverviewRows(rows=[]){
  const ordered=[...rows].sort((left,right)=>{
    const leftTime=Date.parse(left?.updated_at||left?.generated_at||0)||0;
    const rightTime=Date.parse(right?.updated_at||right?.generated_at||0)||0;
  ...
  const identity=ordered[ordered.length-1]||merged;
  const canonicalKey=overviewKey(identity);
  if(canonicalKey)merged.album_key=canonicalKey;
  return merged;
}
function indexOverviewRows(){
  const rows=[...new Set(Object.values(extras.overviews||{}))].filter(row=>row&&typeof row==="object");
  const groups=[];
  rows.forEach(row=>{
    const album={id:row.album_id||row.album_ref||"",title:row.title||"",artist:row.artist||""};
    const group=groups.find(item=>overviewRowsMatchAlbum(item.identity,album));
    if(group)group.rows.push(row);
    else groups.push({identity:album,rows:[row]});
  });
  const indexed={};
  groups.forEach(group=>{
    const row=mergeDuplicateOverviewRows(group.rows);
    const album={id:row.album_id||row.album_ref||group.identity.id||"",title:row.title||group.identity.title||"",artist:row.artist||group.identity.artist||""};
    const keys=new Set([...group.rows.map(item=>item.album_key).filter(Boolean),...overviewKeyCandidates(album)]);
    keys.forEach(key=>{indexed[key]=row});
  });
  extras.overviews=indexed;
  return extras.overviews;
}
function clearLocalOverviewOverridesForAlbum(album){
  try{
    const local=JSON.parse(localStorage.getItem("musicaCustomOverviews")||"{}");
    let changed=false;
    const candidates=new Set(overviewKeyCandidates(album));
    Object.entries(local||{}).forEach(([key,row])=>{
      if(candidates.has(key)||overviewRowsMatchAlbum(row,album)){
        delete local[key];
        changed=true;
      }
    });
    if(changed)localStorage.setItem("musicaCustomOverviews",JSON.stringify(local));
  }catch(error){}
}
...
const albumOverviewRequests=new Map();
const albumCardDetailRequests=new Map();
async function loadAlbumCardDetail(album){
  if(!db||!album||String(album.id||"").startsWith("seed-"))return album;
  if(album.spotify_url!==undefined&&album.summary!==undefined)return album;
  const ref=String(album.id||"");
  if(albumCardDetailRequests.has(ref))return albumCardDetailRequests.get(ref);
  const request=(async()=>{
    const detailDb=publicDataDb||db;
    let result=await detailDb.from("albums").select("id,spotify_url,spotify_id,summary,wikipedia_url,source_url").eq("id",ref).limit(1);
    if(result.error&&/wikipedia_url|source_url|schema cache|column/i.test(result.error.message||""))result=await detailDb.from("albums").select("id,spotify_url,spotify_id,summary").eq("id",ref).limit(1);
    if(result.error){console.warn("[Muze album] card detail could not be loaded",result.error.message||result.error);return album}
    const detail=result.data?.[0]||{};
    state.albums=state.albums.map(item=>String(item.id)===ref?{...item,...detail}:item);
    return {...album,...detail};
  })().finally(()=>albumCardDetailRequests.delete(ref));
  albumCardDetailRequests.set(ref,request);
  return request;
}
async function loadAlbumOverviewForAlbum(album){
  if(!db||!album)return null;
  const ref=String(album.id||"");
  if(albumOverviewRowReadOnly(album)&&Object.keys(albumOverviewRowReadOnly(album)).length>2)return albumOverviewRowReadOnly(album);
  if(albumOverviewRequests.has(ref))return albumOverviewRequests.get(ref);
  const request=(async()=>{
    const overviewDb=publicDataDb||db;
    let result=ref?await overviewDb.from("album_overviews").select("*").eq("album_id",ref).limit(1):{data:[]};
    if((result.error||!result.data?.length)&&album.title){
      result=await overviewDb.from("album_overviews").select("*").eq("title",album.title).eq("artist",album.artist||"").limit(1);
    }
    if(result.error){console.warn("[Muze album] editorial detail could not be loaded",result.error.message||result.error);return null}
    const row=result.data?.[0];
    if(row)cacheOverviewAliases(album,row);
    return row||null;
  })().finally(()=>albumOverviewRequests.delete(ref));
  albumOverviewRequests.set(ref,request);
  return request;
}
async function loadCustomOverviews(){
  ...
  const overviewDb=publicDataDb||db;
  let {data,error}=await overviewDb.from("album_overviews").select(baseOverviewFields+structuredOverviewFields);
  if(error){
    const fallback=await overviewDb.from("album_overviews").select(baseOverviewFields);
    data=fallback.data;
  ...
  if(!error&&data){
    ...
    try{
      const optional=await overviewDb.from("album_overviews").select("album_key,hero_image,moment_image,mood_score");
    ...
    try{
      const genreOptional=await overviewDb.from("album_overviews").select("album_key,manual_genre,overview_focus");
    ...
    try{
      const arcOptional=await overviewDb.from("album_overviews").select("album_key,album_arc");
      if(!arcOptional.error&&arcOptional.data){
        arcOptional.data.forEach(row=>{extras.overviews[row.album_key]={...(extras.overviews[row.album_key]||{}),...row}});
      }
    }catch(arcOptionalError){}
    try{
      const accoladeOptional=await overviewDb.from("album_overviews").select("album_key,year_accolade_rank,year_accolade_year,year_accolade_source,year_accolade_label");
      if(!accoladeOptional.error&&accoladeOptional.data){
        accoladeOptional.data.forEach(row=>{extras.overviews[row.album_key]={...(extras.overviews[row.album_key]||{}),...row}});
      }
    }catch(accoladeOptionalError){}
    try{
      const reviewOptional=await overviewDb.from("album_overviews").select("album_key,review_overview,review_sound,review_impact,review_legacy,review_tagline,review_alternative_taglines,review_defining_moments,review_muze_score,review_minimum_raters,review_closing_verdict,review_mellow_intense_score,review_mellow_intense_explanation,review_generated_at,review_generation_model,review_manual_fields");
    ...
    try{
      const popularTrackOptional=await overviewDb.from("album_overviews").select("album_key,review_most_popular_track");
    ...
    try{
      let published=await overviewDb.from("published_album_reviews").select("album_id,album_key,review,generation_model,prompt_version,quality_score,approved_at");
      if(published.error)published=await overviewDb.from("published_album_reviews").select("album_id,album_key,review,approved_at");
...
async function fetchAlbumMoodScore(album){
  if(!db||!album)return null;
  const cached=albumOverviewRowReadOnly(album)?.mood_score;
  if(cached!==undefined&&cached!==null&&cached!==""&&Number.isFinite(Number(cached)))return Number(cached);
  const lookupFilters=[
...
function albumReviewHtml(row={},album=null){
  ...
  const communityScore=album?score(album):0;
  const scoreHtml=communityScore>0?`<article class="muzeReviewMetric"><span>Muze Score</span><strong>${escapeHtml(communityScore.toFixed(1))}</strong></article>`:"";
  const intensityValue=review.mellowIntenseScore===null?null:Math.max(0,Math.min(100,Number(review.mellowIntenseScore)||0));
  const intensityHtml=intensityValue!==null?`<article class="muzeReviewIntensity mood-scale-card" style="--mood-value:${intensityValue};--w:${intensityValue}%"><div class="mood-scale-head"><span class="mood-scale-heading">Mellow ↔ Intense</span><strong class="mood-scale-value">${escapeHtml(String(Math.round(intensityValue)))}</strong></div><div class="mood-scale-track" role="img" aria-label="Mood intensity ${escapeHtml(String(Math.round(intensityValue)))} out of 100"><span class="mood-scale-fill"></span><span class="mood-scale-ticks" aria-hidden="true"></span><span class="mood-scale-active-ticks" aria-hidden="true"></span><span class="mood-scale-thumb" aria-hidden="true"><span class="mood-scale-thumb-core"></span></span></div><div class="mood-scale-baseline" aria-hidden="true"></div><div class="mood-scale-end-labels"><span class="mood-scale-end-label">Mellow</span><span class="mood-scale-end-label">Intense</span></div>${review.mellowIntenseExplanation?`<p>${escapeHtml(review.mellowIntenseExplanation)}</p>`:""}</article>`:"";
  ...
  return `<section class="muzeAlbumReview"><div class="muzeReviewHead"><span>${review.status==="approved"?"Muze Editorial Review":"Muze Review"}</span>${review.tagline?`<h3>${escapeHtml(review.tagline)}</h3>`:""}</div>${section("Overview",review.overview,"lead")}${section("The Sound",review.sound)}${section("The Impact",review.impact)}${section("The Legacy",review.legacy)}${section("Tagline",review.tagline)}${listSection("Alternative Taglines",review.alternativeTaglines)}${listSection("Defining Moments",review.definingMoments)}${popular}<div class="muzeReviewMetrics">${scoreHtml}</div>${section("Closing Verdict",review.closingVerdict,"verdict")}${intensityHtml}</section>`;
}
function communityAlbumReviewKey(album){return albumRef(album?.id)||overviewKey(album)}
async function loadCommunityAlbumReview(album){
  const key=communityAlbumReviewKey(album);
  if(!key||!db)return {approved:null,pending:null,adminQueue:[]};
  let approved=null,pending=null,adminQueue=[];
  const approvedResult=await db.from("album_review_submissions").select("id,album_id,album_key,username,review_text,status,created_at,reviewed_at").eq("album_id",String(album.id)).eq("submission_type","bio").eq("status","approved").order("reviewed_at",{ascending:false}).limit(1).maybeSingle();
  if(!approvedResult.error)approved=approvedResult.data||null;
  else if(!/album_review_submissions|schema cache|relation/i.test(approvedResult.error.message||""))console.warn("Unable to load approved listener review",approvedResult.error.message||approvedResult.error);
  const user=loggedInUser();
  if(user){
    const pendingResult=await db.from("album_review_submissions").select("id,album_id,album_key,username,review_text,submission_type,status,created_at").eq("album_id",String(album.id)).eq("user_id",user.id).eq("status","pending").order("created_at",{ascending:false});
    if(!pendingResult.error){
      const submissions=Array.isArray(pendingResult.data)?pendingResult.data:[];
      pending=submissions.find(item=>(item.submission_type||"bio")==="bio")||null;
      extras.communityDescriptionSubmissions[key]=Object.fromEntries(submissions.filter(item=>["sound","impact","legacy"].includes(item.submission_type)).map(item=>[item.submission_type,item]));
    }
  }
  extras.communityReviews[key]=approved;
  extras.communityReviewSubmissions[key]=pending;
  if(isAdminUnlocked()){
    const [result,trackResult]=await Promise.all([
      adminOverviewRequest({...adminAlbumPayload(album,"list_album_review_submissions")}),
      adminOverviewRequest({...adminAlbumPayload(album,"list_defining_track_submissions")})
    ]);
    if(result?.ok)adminQueue=Array.isArray(result.rows)?result.rows:[];
    extras.communityReviewAdminQueues[key]=adminQueue;
    extras.communityDefiningTrackAdminQueues[key]=trackResult?.ok&&Array.isArray(trackResult.rows)?trackResult.rows:[];
  }
  return {approved,pending,adminQueue};
}
function communityReviewAdminQueueMarkup(album,rows=[]){
  if(!isAdminUnlocked()||!rows.length)return "";
  const albumId=escapeJsString(album.id);
  return `<div class="albumReviewAdminQueue"><strong>Pending community submissions</strong>${rows.map(row=>`<article><header><b>@${escapeHtml(row.username||"listener")} · ${escapeHtml(row.submission_type||"bio")}</b><small>${escapeHtml(new Date(row.created_at||Date.now()).toLocaleDateString())}</small></header><p>${formatParagraphText(row.review_text||"")}</p><div><button type="button" onclick="moderateCommunityAlbumReview('${albumId}','${escapeJsString(row.id)}','approve')">Approve</button><button type="button" onclick="moderateCommunityAlbumReview('${albumId}','${escapeJsString(row.id)}','reject')">Reject</button></div></article>`).join("")}</div>`;
}
function definingTrackAdminQueueMarkup(album,rows=[]){
  if(!isAdminUnlocked()||!rows.length)return "";
  const albumId=escapeJsString(album.id);
  return `<div class="albumReviewAdminQueue definingTrackAdminQueue"><strong>Pending defining-track submissions</strong>${rows.map(row=>`<article><header><b>@${escapeHtml(row.username||"listener")}</b><small>${escapeHtml(new Date(row.created_at||Date.now()).toLocaleDateString())}</small></header><p>${normalizeOverviewList(row.track_names).map((name,index)=>`${index+1}. ${escapeHtml(name)}`).join("<br>")}</p><div><button type="button" onclick="moderateDefiningTrackSubmission('${albumId}','${escapeJsString(row.id)}','approve')">Approve</button><button type="button" onclick="moderateDefiningTrackSubmission('${albumId}','${escapeJsString(row.id)}','reject')">Reject</button></div></article>`).join("")}</div>`;
}
function emptyAlbumReviewMarkup(album,pending=null,adminQueue=[]){
  const albumId=escapeJsString(album.id);
  const pendingText=pending?`<span class="albumReviewPending">Your bio is awaiting Muze approval.</span>`:"";
  return `<div class="albumReviewContribution"><strong>No bio added yet.</strong><span>Be the first to add a Muze bio.</span>${pendingText}<button type="button" onclick="openCommunityAlbumReviewForm(this,'${albumId}')">${pending?"Edit your submission":"Write a bio"}</button><div class="albumReviewSubmissionForm" hidden><label for="communityAlbumReviewText">Your bio</label><textarea id="communityAlbumReviewText" minlength="80" maxlength="4000" placeholder="Write a bio for this album…">${escapeHtml(pending?.review_text||"")}</textarea><small>80–4,000 characters. Your bio will appear only after Muze admin approval.</small><div><button type="button" onclick="submitCommunityAlbumReview(this,'${albumId}')">Submit for approval</button><button type="button" onclick="this.closest('.albumReviewSubmissionForm').hidden=true">Cancel</button></div><em class="albumReviewSubmissionStatus"></em></div>${communityReviewAdminQueueMarkup(album,adminQueue.filter(row=>(row.submission_type||"bio")==="bio"))}</div>`;
}
function emptyOverviewDescriptionMarkup(album,type){
  const labels={sound:"sound",impact:"impact",legacy:"legacy"};
...
window.openOverviewDescriptionForm=function(button,albumId,type){
  ...
  const form=button?.closest(".overviewDescriptionContribution")?.querySelector(".overviewDescriptionForm");
  if(form){form.hidden=false;form.querySelector("textarea")?.focus()}
};
window.submitOverviewDescription=async function(button,albumId,type){
  const user=loggedInUser();
  if(!user){openAuthModal("Log in or sign up to submit a description.");return}
  const album=state.albums.find(item=>String(item.id)===String(albumId));
  const form=button?.closest(".overviewDescriptionForm");
  const status=form?.querySelector(".overviewDescriptionStatus");
  const description=String(form?.querySelector("textarea")?.value||"").replace(/\s+/g," ").trim();
  if(description.length<80){if(status)status.textContent="Please write at least 80 characters.";return}
  if(!db||!album||!["sound","impact","legacy"].includes(type)){if(status)status.textContent="Description submission is unavailable right now.";return}
  button.disabled=true;if(status)status.textContent="Submitting…";
  const key=communityAlbumReviewKey(album);
  const existing=extras.communityDescriptionSubmissions[key]?.[type];
  const payload={album_id:String(album.id),album_key:overviewKey(album),album_title:String(album.title||""),artist_name:String(album.artist||""),user_id:user.id,username:currentUsername()||authDisplayName()||"Listener",review_text:description,submission_type:type,status:"pending",updated_at:new Date().toISOString()};
  const result=existing?.id?await db.from("album_review_submissions").update(payload).eq("id",existing.id).select().single():await db.from("album_review_submissions").insert({...payload,created_at:new Date().toISOString()}).select().single();
  button.disabled=false;
  if(result.error){if(status)status.textContent=/submission_type|schema cache/i.test(result.error.message||"")?"The description-submission database migration still needs to be applied.":result.error.message||"Description submission failed.";return}
  extras.communityDescriptionSubmissions[key]={...(extras.communityDescriptionSubmissions[key]||{}),[type]:result.data};
  if(status)status.textContent="Submitted for Muze approval.";
  setTimeout(()=>openAlbum(album.id),350);
};
window.openCommunityAlbumReviewForm=function(button,albumId){
  if(!loggedInUser()){openAuthModal("Log in or sign up to submit an album review for Muze approval.");return}
  const form=button?.closest(".albumReviewContribution")?.querySelector(".albumReviewSubmissionForm");
  if(form){form.hidden=false;form.querySelector("textarea")?.focus()}
};
window.submitCommunityAlbumReview=async function(button,albumId){
  const user=loggedInUser();
  if(!user){openAuthModal("Log in or sign up to submit an album review.");return}
  const album=state.albums.find(item=>String(item.id)===String(albumId));
  const form=button?.closest(".albumReviewSubmissionForm");
  const textarea=form?.querySelector("textarea");
  const status=form?.querySelector(".albumReviewSubmissionStatus");
  const reviewText=String(textarea?.value||"").replace(/\s+/g," ").trim();
  if(reviewText.length<80){if(status)status.textContent="Please write at least 80 characters.";return}
  if(!db||!album){if(status)status.textContent="Review submission is unavailable right now.";return}
  button.disabled=true;if(status)status.textContent="Submitting…";
  const key=communityAlbumReviewKey(album);
  const existing=extras.communityReviewSubmissions[key];
  const payload={album_id:String(album.id),album_key:overviewKey(album),album_title:String(album.title||""),artist_name:String(album.artist||""),user_id:user.id,username:currentUsername()||authDisplayName()||"Listener",review_text:reviewText,submission_type:"bio",status:"pending",updated_at:new Date().toISOString()};
  const result=existing?.id?await db.from("album_review_submissions").update(payload).eq("id",existing.id).select().single():await db.from("album_review_submissions").insert({...payload,created_at:new Date().toISOString()}).select().single();
  button.disabled=false;
  if(result.error){if(status)status.textContent=/album_review_submissions|schema cache|relation/i.test(result.error.message||"")?"The review-submission database migration still needs to be applied.":result.error.message||"Review submission failed.";return}
  extras.communityReviewSubmissions[key]=result.data;
  if(status)status.textContent="Submitted for Muze approval.";
  setTimeout(()=>openAlbum(album.id),350);
};
window.moderateCommunityAlbumReview=async function(albumId,submissionId,decision){
  const album=state.albums.find(item=>String(item.id)===String(albumId));
  if(!album||!isAdminUnlocked())return;
  const result=await adminOverviewRequest({...adminAlbumPayload(album,decision==="approve"?"approve_album_review_submission":"reject_album_review_submission"),submission_id:submissionId});
  if(!result?.ok)return;
  await openAlbum(album.id);
};
window.moderateDefiningTrackSubmission=async function(albumId,submissionId,decision){
  const album=state.albums.find(item=>String(item.id)===String(albumId));
  if(!album||!isAdminUnlocked())return;
  const result=await adminOverviewRequest({...adminAlbumPayload(album,decision==="approve"?"approve_defining_track_submission":"reject_defining_track_submission"),submission_id:submissionId});
  if(!result?.ok)return;
  if(decision==="approve"&&result.overview){extras.overviews[overviewKey(album)]={...albumOverviewRowReadOnly(album),...result.overview};indexOverviewRows()}
  await openAlbum(album.id);
};
...
function albumHasPublishedReview(album,row={},manualDescription=""){
  if(String(manualDescription||"").trim())return true;
  if(hasAlbumReview(row))return true;
  const simple=albumSimpleTitleByArtist(album).replace(/\s+/g," ").trim().toLowerCase();
  const candidates=[row.intro_summary,row.overview,row.source_summary]
    .map(value=>String(value||"").replace(/\s+/g," ").trim())
    .filter(Boolean);
  if(row.fallback_generated===true)return false;
  return candidates.some(value=>value.toLowerCase()!==simple&&!genericOverviewText(value));
}
...
function albumOverviewHtml(a,{albumId,coverUrl,albumScore,total,customOverview,canEditOverview}){
  ...
  const soundContribution=sound===placeholder.sound_summary?emptyOverviewDescriptionMarkup(a,"sound"):"";
  const impactContribution=impact===placeholder.impact_summary?emptyOverviewDescriptionMarkup(a,"impact"):"";
  const legacyContribution=legacy===placeholder.legacy_summary?emptyOverviewDescriptionMarkup(a,"legacy"):"";
  ...
  host.innerHTML=names.length?names.map(name=>`<button type="button" class="overviewMomentChip" data-track-name="${escapeHtml(name)}" onclick="playOverviewMomentPreview('${escapeJsString(albumId)}','${escapeJsString(name)}',this)">${escapeHtml(name)}</button>`).join(""):`<div class="defining-tracks-empty"><span>No defining tracks yet.</span><button type="button" class="defining-tracks-add" onclick="openDefiningTracksContribution('${escapeJsString(albumId)}')">+ Add</button></div>`;
}
window.closeDefiningTracksContribution=function(){document.getElementById("definingTracksContributionModal")?.remove()};
window.openDefiningTracksContribution=async function(albumId){
  if(!requireAuth("review",()=>window.openDefiningTracksContribution(albumId)))return;
  const album=state.albums.find(item=>String(item.id)===String(albumId));
  const user=loggedInUser();
  ...
  const trackNames=(tracks||[]).map(track=>String(track.name||"").trim()).filter(Boolean);
  if(trackNames.length<5){alert("This album does not have five available tracks to choose from.");return}
  let existing=null;
  if(db){
    const result=await db.from("album_defining_track_submissions").select("id,track_names,status,created_at").eq("album_id",String(album.id)).eq("user_id",user.id).eq("status","pending").order("created_at",{ascending:false}).limit(1).maybeSingle();
    if(!result.error)existing=result.data||null;
  }
  const selected=new Set(normalizeOverviewList(existing?.track_names));
  closeDefiningTracksContribution();
  const modal=document.createElement("div");
  modal.id="definingTracksContributionModal";
  modal.className="modal definingTracksContributionModal";
  modal.addEventListener("click",event=>{if(event.target===modal)closeDefiningTracksContribution()});
  modal.innerHTML=`<div class="modalPanel definingTracksContributionPanel" role="dialog" aria-modal="true" aria-labelledby="definingTracksContributionTitle"><button type="button" class="close" aria-label="Close" onclick="closeDefiningTracksContribution()">&times;</button><p class="eyebrow">Community contribution</p><h2 id="definingTracksContributionTitle">Choose five defining tracks</h2><p>Select exactly five tracks from ${escapeHtml(album.title||"this album")}. Muze moderators will review your submission.</p><div class="definingTracksSelectionList">${trackNames.map((name,index)=>`<label><input type="checkbox" value="${escapeHtml(name)}" ${selected.has(name)?"checked":""} onchange="changeDefiningTrackSelection(this)"><span><b>${String(index+1).padStart(2,"0")}</b>${escapeHtml(name)}</span></label>`).join("")}</div><div class="definingTracksSelectionFoot"><strong id="definingTracksSelectionCount">${selected.size}/5 selected</strong><div><button type="button" class="secondary" onclick="closeDefiningTracksContribution()">Cancel</button><button type="button" id="submitDefiningTracksButton" onclick="submitDefiningTracksContribution(this,'${escapeJsString(album.id)}','${escapeJsString(existing?.id||"")}')" ${selected.size===5?"":"disabled"}>${existing?"Update submission":"Submit tracks"}</button></div></div><em id="definingTracksContributionStatus"></em></div>`;
  document.body.appendChild(modal);
  changeDefiningTrackSelection();
};
window.changeDefiningTrackSelection=function(input){
  const modal=document.getElementById("definingTracksContributionModal");
  if(!modal)return;
  let checked=[...modal.querySelectorAll('.definingTracksSelectionList input:checked')];
  if(checked.length>5&&input){input.checked=false;checked=[...modal.querySelectorAll('.definingTracksSelectionList input:checked')]}
  modal.querySelectorAll('.definingTracksSelectionList input:not(:checked)').forEach(item=>item.disabled=checked.length>=5);
  const count=modal.querySelector("#definingTracksSelectionCount");if(count)count.textContent=`${checked.length}/5 selected`;
  const submit=modal.querySelector("#submitDefiningTracksButton");if(submit)submit.disabled=checked.length!==5;
};
window.submitDefiningTracksContribution=async function(button,albumId,existingId=""){
  const album=state.albums.find(item=>String(item.id)===String(albumId));
  const user=loggedInUser();
  const modal=document.getElementById("definingTracksContributionModal");
  const status=modal?.querySelector("#definingTracksContributionStatus");
  const trackNames=[...(modal?.querySelectorAll('.definingTracksSelectionList input:checked')||[])].map(input=>input.value);
  if(trackNames.length!==5){if(status)status.textContent="Select exactly five tracks.";return}
  if(!album||!user||!db){if(status)status.textContent="Track submissions are unavailable right now.";return}
  button.disabled=true;if(status)status.textContent="Submitting…";
  const payload={album_id:String(album.id),album_key:overviewKey(album),album_title:String(album.title||""),artist_name:String(album.artist||""),user_id:user.id,username:currentUsername()||authDisplayName()||"Listener",track_names:trackNames,status:"pending",updated_at:new Date().toISOString()};
  const result=existingId?await db.from("album_defining_track_submissions").update(payload).eq("id",existingId).eq("user_id",user.id).eq("status","pending").select().single():await db.from("album_defining_track_submissions").insert({...payload,created_at:new Date().toISOString()}).select().single();
  button.disabled=false;
  if(result.error){if(status)status.textContent=/album_defining_track_submissions|schema cache|PGRST205/i.test(result.error.message||"")?"The defining-track submission migration still needs to be applied.":result.error.message||"Track submission failed.";return}
  closeDefiningTracksContribution();
  const host=document.getElementById("overviewMomentChips");
  if(host)host.insertAdjacentHTML("afterend",'<p class="definingTracksContributionConfirmation">Tracks submitted for review.</p>');
};
...
function muzeOverallRank(album){
  if(!state.albumCatalogueReady||!album||score(album)<=0)return null;
  const ranked=state.albums.filter(item=>score(item)>0).slice().sort((a,b)=>score(b)-score(a)||count(b)-count(a)||String(a.title||"").localeCompare(String(b.title||"")));
  const albumKey=albumIdentityKey(album)||String(album.id||"");
  const index=ranked.findIndex(item=>(albumIdentityKey(item)||String(item.id||""))===albumKey);
  return index>=0?index+1:null;
}
function muzeTop250Rank(album){
  const value=album?.muze_rank??album?.muze_all_time_rank??album?.ranking??muzeOverallRank(album);
...
function albumBackCoverOverrideUrl(album){
  const text=normalizeAlbumName(`${album?.title||""} ${album?.artist||""}`);
  if(text.includes("let there be rock")&&(text.includes("ac dc")||text.includes("acdc")))return "https://r2.theaudiodb.com/images/media/album/back/svqrut1538863907.jpg";
...
let albumLookupCacheSource=null;
let albumLookupCacheLength=0;
let albumLookupCache={byId:new Map(),bySpotify:new Map(),byIdentity:new Map(),byCover:new Map()};
function albumLookupMaps(){
  if(albumLookupCacheSource===state.albums&&albumLookupCacheLength===state.albums.length)return albumLookupCache;
  const byId=new Map(),bySpotify=new Map(),byIdentity=new Map(),byCover=new Map();
  (state.albums||[]).forEach(album=>{
    const id=String(album?.id||"").trim().toLowerCase();
    const spotify=String(album?.spotify_id||album?.spotifyId||"").trim().toLowerCase();
    const identity=albumIdentityKey(album);
    const cover=coverKey(album);
    if(id&&!byId.has(id))byId.set(id,album);
    if(spotify&&!bySpotify.has(spotify))bySpotify.set(spotify,album);
    if(identity&&!byIdentity.has(identity))byIdentity.set(identity,album);
    if(cover&&!byCover.has(cover))byCover.set(cover,album);
  });
  albumLookupCacheSource=state.albums;
  albumLookupCacheLength=state.albums.length;
  albumLookupCache={byId,bySpotify,byIdentity,byCover};
  return albumLookupCache;
}
function fastAlbumMatch(album){
  const maps=albumLookupMaps();
  const id=String(album?.id||"").trim().toLowerCase();
  const spotify=String(album?.spotify_id||album?.spotifyId||"").trim().toLowerCase();
  const identity=albumIdentityKey(album);
  const cover=coverKey(album);
  return (id&&maps.byId.get(id))||(spotify&&maps.bySpotify.get(spotify))||(identity&&maps.byIdentity.get(identity))||(cover&&maps.byCover.get(cover))||null;
}
function existingAlbumMatch(album){return fastAlbumMatch(album)||state.albums.find(a=>isSameAlbum(a,album))}
...
function trackCommentButtonHtml(albumId,key,trackName,count=0){
  ...
  return `<button class="trackDots trackCommentBubble" onclick="openTrackComments('${escapeJsString(albumId)}','${escapeJsString(key)}','${escapeJsString(trackName)}')" aria-label="Open ${safeCount} comment${safeCount===1?"":"s"} for ${escapeHtml(trackName)}"><span class="trackCommentEmoji" aria-hidden="true">&#128172;&#xfe0e;</span>${badge}</button>`;
}
...
function trackDurationText(track){
  const duration=Math.max(0,Number(track?.duration_ms)||0);
  if(!duration)return "";
  return Math.floor(duration/60000)+":"+String(Math.floor((duration%60000)/1000)).padStart(2,"0");
}
function trackCommunityScoreValue(score){
  if(!score||Number(score.ratings_count)<=0)return null;
  const value=Number(score.avg_rating);
...
function trackRatingStarsHtml(score,label="Track"){
  const value=Number.isFinite(Number(score))?Math.max(0,Math.min(10,Number(score))):null;
  const fiveStarValue=value===null?0:value/2;
  const stars=Array.from({length:5},(_,index)=>{
    const fill=Math.max(0,Math.min(1,fiveStarValue-index));
    return `<i style="--track-star-fill:${Math.round(fill*100)}%" aria-hidden="true"></i>`;
  }).join("");
  const aria=value===null?`${label} has no community score yet`:`${label} community rating ${value.toFixed(1)} out of 10`;
  return `<span class="trackRatingStars" role="img" aria-label="${escapeHtml(aria)}">${stars}</span>`;
}
...
function trackSideLabel(track){
  const raw=String(track?.side_label||track?.vinyl_side||track?.side_name||track?.side||"").trim();
  if(!raw)return "";
  if(/^side\s+/i.test(raw))return raw;
  return `Side ${raw}`;
}
function albumArcEditorialData(savedOverview={},album={}){
  let raw=savedOverview.album_arc??album.album_arc??null;
  if(typeof raw==="string"){
    try{raw=JSON.parse(raw)}catch(error){return null}
  }
  if(!raw||typeof raw!=="object"||!Array.isArray(raw.phases))return null;
  const phases=raw.phases.map(phase=>{
    const label=String(phase?.label||"").trim();
    const start=Math.max(1,Math.round(Number(phase?.start_track)||0));
    const end=Math.max(start,Math.round(Number(phase?.end_track)||start));
    return label&&start?{label,start,end}:null;
  }).filter(Boolean);
  if(!phases.length)return null;
  return {title:String(raw.title||"").trim(),phases};
}
function albumArcHtml(arc){
  if(!arc)return "";
  const phases=arc.phases.map(phase=>{
    const range=phase.start===phase.end?`Track ${phase.start}`:`Tracks ${phase.start}&ndash;${phase.end}`;
    return `<li><span aria-hidden="true"></span><strong>${escapeHtml(phase.label)}</strong><small>${range}</small></li>`;
  }).join("");
  return `<section class="albumJourneyArc" style="--arc-count:${arc.phases.length}"><div class="albumJourneyArcCopy"><p>The album arc</p>${arc.title?`<h3>${escapeHtml(arc.title)}</h3>`:""}</div><ol>${phases}</ol></section>`;
}
function moreLikeThisProfileTokens(album){
  const overview=albumOverviewRowReadOnly(album)||{};
  const source=[albumGenreLabel(album),genreSearchText(album),overview.mood_tags,overview.vibe_tags,overview.sound_summary,overview.overview_focus].flat().filter(Boolean).join(" ");
...
function moreLikeThisSimilarity(album,item){
  ...
  const currentTokens=moreLikeThisProfileTokens(album);
  const itemTokens=moreLikeThisProfileTokens(item);
  let similarity=currentGenre&&itemGenre===currentGenre?18:0;
  currentTokens.forEach(token=>{if(itemTokens.has(token))similarity+=4});
  const currentYear=Number(albumReleaseYear(album)||album?.year||0);
  const itemYear=Number(albumReleaseYear(item)||item?.year||0);
  if(currentYear&&itemYear){
    const distance=Math.abs(currentYear-itemYear);
    similarity+=distance<=3?5:distance<=8?3:distance<=15?1:0;
  }
  return similarity+Math.min(2,Math.max(0,score(item)-7)/1.5);
}
const curatedMoreLikeThisReplacements={
...
function applyCuratedMoreLikeThisReplacements(album,selected){
  ...
  replacements.forEach(replacement=>{
    ...
    if(index<0){
      const sourceArtist=normalizeArtistKey(album.artist);
      index=result.findIndex(item=>normalizeArtistKey(item.artist)===sourceArtist);
    }
    if(index<0&&result.length)index=result.length-1;
...
function moreLikeThisAlbums(album,limit=4){
  ...
  for(const entry of candidates){
    const artist=normalizeArtistKey(entry.item.artist);
    if((artistCounts.get(artist)||0)>=1&&candidates.length>limit)continue;
    selected.push(entry.item);
    artistCounts.set(artist,(artistCounts.get(artist)||0)+1);
    if(selected.length===limit)break;
  }
  if(selected.length<limit){
    for(const entry of candidates){if(!selected.includes(entry.item)){selected.push(entry.item);if(selected.length===limit)break}}
  }
...
function cachedAlbumRecommendations(albumId){
  try{
    const cached=JSON.parse(localStorage.getItem(ALBUM_RECOMMENDATION_CACHE_PREFIX+albumRef(albumId))||"null");
    if(!cached||!Array.isArray(cached.rows)||Date.now()-Number(cached.saved_at||0)>1000*60*60*24*60)return [];
    return cached.rows;
  }catch(error){return []}
}
function storeAlbumRecommendations(albumId,rows){
  try{localStorage.setItem(ALBUM_RECOMMENDATION_CACHE_PREFIX+albumRef(albumId),JSON.stringify({saved_at:Date.now(),rows}))}catch(error){}
}
function acceptAlbumRecommendations(album,rows,{rerender=true}={}){
...
async function hydrateAlbumRecommendations(album){
  ...
  return extras.albumRecommendationRequests[ref];
}
const verifiedAlbumInfluenceRelationships={
  "the beatles::abbey road":{
    earlier:[
      {title:"Pet Sounds",artist:"The Beach Boys",year:1966,source:"https://www.britannica.com/topic/Pet-Sounds"},
      {title:"Music from Big Pink",artist:"The Band",year:1968,source:"https://www.loc.gov/static/programs/national-recording-preservation-board/documents/MusicFromBigPink.pdf"},
      {title:"John Wesley Harding",artist:"Bob Dylan",year:1967,source:"https://www.bobdylan.com/albums/john-wesley-harding/"}
    ],
    later:[
      {title:"McLemore Avenue",artist:"Booker T. & the M.G.'s",year:1970,source:"https://staxrecords.com/artist/booker-t-the-mgs/"},
      {title:"The Other Side of Abbey Road",artist:"George Benson",year:1970,source:"https://www.georgebenson.com/"}
    ]
  }
};
function verifiedInfluenceKey(album){return `${normalizeArtistKey(album?.artist)}::${normalizeOverviewTitle(clientCleanAlbumTitle(album?.title||""))}`}
...
function spotifyInfluenceMatch(results,reference){
  const wantedTitle=normalizeOverviewTitle(clientCleanAlbumTitle(reference?.title||""));
  const wantedArtist=normalizeArtistKey(reference?.artist||"");
  const wantedYear=Number(reference?.year)||0;
  return (results||[]).map(album=>{
    const title=normalizeOverviewTitle(clientCleanAlbumTitle(album?.title||""));
    const artist=normalizeArtistKey(album?.artist||"");
    if(title!==wantedTitle||artist!==wantedArtist)return null;
    const year=Number(album?.year)||0;
    const editionPenalty=/deluxe|expanded|anniversary|collector|special|super deluxe|legacy|bonus/i.test(String(album?.title||""))?20:0;
    const yearScore=wantedYear&&year===wantedYear?40:wantedYear&&year?Math.max(0,20-Math.abs(year-wantedYear)*4):0;
    return {album,score:100+yearScore-editionPenalty};
  }).filter(Boolean).sort((a,b)=>b.score-a.score)[0]?.album||null;
}
async function importVerifiedInfluenceAlbum(reference){
  ...
  const data=await res.json().catch(()=>({}));
  if(!res.ok)throw new Error(data.error||"Spotify album search failed.");
  const spotifyAlbum=spotifyInfluenceMatch(data.albums,reference);
  ...
  try{
    ...
    return reconcilePendingSpotifyAlbum(pending.id,saved);
  }catch(error){
    removePendingSpotifyAlbum(pending.id);
    throw error;
  }
}
window.openOrImportInfluenceAlbum=async function(button,title,artist,year){
  if(button?.dataset.importing==="true")return;
  const reference={title,artist,year:Number(year)||year};
  const originalLabel=button?.getAttribute("aria-label")||"";
  if(button){
    button.dataset.importing="true";
    button.classList.add("isImporting");
    button.setAttribute("aria-busy","true");
    button.setAttribute("aria-label",`Adding ${title} by ${artist} to Muze from Spotify`);
  }
  setAdminInlineStatus(`Finding ${title} by ${artist} on Spotify...`,"");
  try{
    const album=await importVerifiedInfluenceAlbum(reference);
    if(!album?.id)throw new Error("The album was found but could not be added to Muze.");
    setAdminInlineStatus(`${album.title} was added to Muze from Spotify.`,"success");
    openAlbum(album.id);
  }catch(error){
    console.error("Muze influence album import failed",error);
    setAdminInlineStatus(error.message||"This album could not be added from Spotify.","error");
    if(button){
      button.dataset.importing="false";
      button.classList.remove("isImporting");
...
function bindAlbumLegacyCarousel(host){
  ...
  const viewport=section?.querySelector(".albumLegacyViewport");
  if(!section||!viewport)return;
  section.querySelectorAll("[data-legacy-direction]").forEach(button=>button.addEventListener("click",()=>{
    const direction=Number(button.dataset.legacyDirection)||1;
    const amount=Math.max(1,viewport.clientWidth+10);
    const maximum=Math.max(0,viewport.scrollWidth-viewport.clientWidth);
    const next=Math.max(0,Math.min(maximum,viewport.scrollLeft+direction*amount));
...
function renderTrackList(albumId,suppliedTracks=null){
  ...
  const definingTrackNames=normalizeOverviewList(savedOverview.defining_tracks).slice(0,5);
  const otherDefiningTrackNames=definingTrackNames.filter(name=>!sameCommentTrackKey(name,first.name)).slice(0,4);
  let previousSide="";
  const rows=visibleTracks.map((track,i)=>{
    ...
    const side=trackSideLabel(track);
    const sideMarker=side&&side!==previousSide?`<div class="trackJourneySide"><span>${escapeHtml(side.replace(/^side\s*/i,"")||side)}</span><b>${escapeHtml(side)}</b><i aria-hidden="true"></i></div>`:"";
  ...
  const mobileRows=visibleTracks.map((track,i)=>{
    const key=trackKey(track);
    const current=ratings[key]||localTrackRating(albumId,key);
    const scoreValue=trackCommunityScoreValue(songScores[key]);
...
window.openTrackRating=function(albumId,trackKeyValue,trackName){
  ...
  const ratingAlbum=state.albums.find(album=>String(album.id)===String(albumId)||albumRef(album.id)===albumRef(albumId))||{};
  const ratingArtist=String(ratingAlbum.artist||ratingAlbum.artist_name||"").trim();
  ...
  popup.innerHTML=`<div class="trackRatingPanel albumRatingPanel songStarRatingPanel"><button class="close" onclick="closeTrackRating()">&times;</button><p class="eyebrow">Song Rating</p><h3>${escapeHtml(trackName)}</h3>${ratingArtist?`<p class="songRatingArtist">${escapeHtml(ratingArtist)}</p>`:""}<div class="albumStarRating" role="slider" tabindex="0" aria-label="Rate ${escapeHtml(trackName)} out of 10 stars" aria-valuemin="0.5" aria-valuemax="10" aria-valuestep="0.5" aria-valuenow="${Number(current)||.5}" aria-valuetext="${current?`${Number(current)} out of 10 stars`:"Not rated"}" data-current="${Number(current)||0}" data-value="${Number(current)||0}">${stars}</div><div class="ratingDetailsBlock"><button class="linkBtn" onclick="toggleRatingDetails('trackRatingDetails',this)">See ratings</button><div id="trackRatingDetails" class="hidden"><div class="emptyMini">Loading ratings...</div></div></div></div>`;
  bindTrackStarRating(popup.querySelector(".albumStarRating"),albumId,trackKeyValue,trackName);
...
function bindTrackStarRating(control,albumId,trackKeyValue,trackName){
  if(!control)return;
  control.addEventListener("pointermove",event=>{
    if(event.pointerType==="mouse")paintAlbumStarRating(control,albumStarValueFromPointer(event,control));
  });
  control.addEventListener("click",event=>{
    ...
    if(value){rateTrack(albumId,trackKeyValue,trackName,value);closeTrackRating()}
  });
  control.addEventListener("pointerleave",()=>{
    paintAlbumStarRating(control,Number(control.dataset.current||0));
  });
  control.addEventListener("keydown",event=>handleTrackStarRatingKey(event,albumId,trackKeyValue,trackName));
}
...
window.startTrackStarDrag=function(event){
  if(event.button!==undefined&&event.button!==0)return;
  const control=event.currentTarget;
  event.preventDefault();
  control.dataset.dragging="true";
...
window.finishTrackStarDrag=function(event,albumId,trackKeyValue,trackName){
  const control=event.currentTarget;
  if(control.dataset.dragging!=="true")return;
  event.preventDefault();
  paintAlbumStarRating(control,albumStarValueFromPointer(event,control));
...
window.handleTrackStarRatingKey=function(event,albumId,trackKeyValue,trackName){
  const control=event.currentTarget;
  let value=Number(control.dataset.value||control.dataset.current||0);
  if(event.key==="ArrowRight"||event.key==="ArrowUp")value=Math.min(10,(value||0)+.5);
  else if(event.key==="ArrowLeft"||event.key==="ArrowDown")value=Math.max(.5,(value||1)-.5);
  ...
  else if((event.key==="Enter"||event.key===" ")&&value){event.preventDefault();rateTrack(albumId,trackKeyValue,trackName,value);closeTrackRating();return}
  else return;
  event.preventDefault();
  paintAlbumStarRating(control,value);
};
...
const albumPerimeterContrastCache=new Map();
const albumPerimeterPaletteCache=new Map();
function albumCoverPerimeterPalette(source){
  const src=String(source||"").trim();
  if(!src)return Promise.resolve(null);
  if(albumPerimeterPaletteCache.has(src))return Promise.resolve(albumPerimeterPaletteCache.get(src));
  return new Promise(resolve=>{
    const finish=value=>{albumPerimeterPaletteCache.set(src,value);resolve(value)};
    ...
    image.onload=()=>{
      try{
        ...
        const pixels=context.getImageData(0,0,44,44).data;
        const buckets=new Map();
        for(let index=0;index<pixels.length;index+=4){
          if(pixels[index+3]<180)continue;
          const red=pixels[index],green=pixels[index+1],blue=pixels[index+2];
          const high=Math.max(red,green,blue)/255,low=Math.min(red,green,blue)/255;
          const lightness=(high+low)/2,chroma=high-low;
          if(lightness<.055||lightness>.95)continue;
          const saturation=chroma===0?0:chroma/(1-Math.abs(2*lightness-1));
          const key=`${red>>5}-${green>>5}-${blue>>5}`;
          const weight=.32+(saturation*1.75)+(lightness>.16&&lightness<.82?.42:0);
          const bucket=buckets.get(key)||{red:0,green:0,blue:0,weight:0,count:0};
          bucket.red+=red*weight;bucket.green+=green*weight;bucket.blue+=blue*weight;
          bucket.weight+=weight;bucket.count++;
          buckets.set(key,bucket);
        }
        let selected=null,selectedScore=-1;
        buckets.forEach(bucket=>{
          const red=bucket.red/bucket.weight,green=bucket.green/bucket.weight,blue=bucket.blue/bucket.weight;
          const high=Math.max(red,green,blue)/255,low=Math.min(red,green,blue)/255;
          const lightness=(high+low)/2,chroma=high-low;
          const saturation=chroma===0?0:chroma/(1-Math.abs(2*lightness-1));
          const score=bucket.weight*(.68+saturation*.82)*(1-Math.abs(lightness-.5)*.36);
          if(score>selectedScore){selectedScore=score;selected={red,green,blue}}
        });
        if(!selected){finish(null);return}
        const selectedHigh=Math.max(selected.red,selected.green,selected.blue)/255;
        const selectedLow=Math.min(selected.red,selected.green,selected.blue)/255;
        const selectedChroma=selectedHigh-selectedLow;
        const selectedLightness=(selectedHigh+selectedLow)/2;
        const selectedSaturation=selectedChroma===0?0:selectedChroma/(1-Math.abs(2*selectedLightness-1));
        let selectedHue=0;
        if(selectedChroma){
          if(selectedHigh===selected.red/255)selectedHue=60*((((selected.green-selected.blue)/255)/selectedChroma)%6);
          else if(selectedHigh===selected.green/255)selectedHue=60*(((selected.blue-selected.red)/255)/selectedChroma+2);
          else selectedHue=60*(((selected.red-selected.green)/255)/selectedChroma+4);
          if(selectedHue<0)selectedHue+=360;
        }
        const harshWarm=(selectedHue>=345||selectedHue<45)&&selectedSaturation>.38;
        const primary=[selected.red,selected.green,selected.blue].map(value=>Math.round(value));
        const soft=primary.map(value=>Math.round(value+(255-value)*.18));
        finish({primary:primary.join(","),soft:soft.join(","),harshWarm});
      }catch(error){finish(null)}
    };
    image.src=src;
  });
}
function albumCoverHasStrongContrast(source){
  const src=String(source||"").trim();
  if(!src)return Promise.resolve(false);
  if(albumPerimeterContrastCache.has(src))return Promise.resolve(albumPerimeterContrastCache.get(src));
  return new Promise(resolve=>{
    const finish=value=>{albumPerimeterContrastCache.set(src,Boolean(value));resolve(Boolean(value))};
    const image=new Image();
    image.crossOrigin="anonymous";
    image.referrerPolicy="no-referrer";
    ...
    image.onload=()=>{
      try{
        ...
        const luminance=[];
        let chromaTotal=0,vividPixels=0,clashWeight=0,samples=0;
        for(let index=0;index<pixels.length;index+=4){
          if(pixels[index+3]<180)continue;
          ...
          let hue=0;
          if(chroma){
            if(high===red)hue=60*(((green-blue)/chroma)%6);
            else if(high===green)hue=60*((blue-red)/chroma+2);
            else hue=60*((red-green)/chroma+4);
            if(hue<0)hue+=360;
          }
          luminance.push(.2126*red+.7152*green+.0722*blue);
          chromaTotal+=chroma;
          if(chroma>.42)vividPixels++;
          const lightRed=(hue>=345||hue<14)&&saturation>.25&&lightness>.58;
          const primaryRed=(hue>=350||hue<10)&&saturation>.62&&lightness>.30&&lightness<.72;
          const orange=hue>=14&&hue<43&&saturation>.56&&lightness>.36;
          const peachCoral=(hue>=4&&hue<38)&&saturation>.28&&lightness>.61;
          const pastelYellow=hue>=43&&hue<67&&saturation>.28&&lightness>.68;
          const limeChartreuse=hue>=67&&hue<112&&saturation>.40&&lightness>.40;
          const lightGreen=hue>=96&&hue<158&&saturation>.24&&lightness>.60;
          const mintAqua=hue>=148&&hue<198&&saturation>.24&&lightness>.48;
          const babyBlueCyan=hue>=188&&hue<224&&saturation>.23&&lightness>.57;
          const primaryBlue=hue>=218&&hue<252&&saturation>.58&&lightness>.28&&lightness<.72;
          const lavender=hue>=250&&hue<292&&saturation>.20&&lightness>.61;
          const brightPinkFuchsia=hue>=292&&hue<345&&saturation>.45&&lightness>.43;
          const coolLightGray=hue>=190&&hue<250&&saturation>=.045&&saturation<.24&&lightness>.66;
          const severe=primaryRed||orange||limeChartreuse||mintAqua||primaryBlue||brightPinkFuchsia;
          const paletteClash=severe||lightRed||peachCoral||pastelYellow||lightGreen||babyBlueCyan||lavender;
          if(severe)clashWeight+=1.45;
          else if(paletteClash)clashWeight+=1;
          else if(coolLightGray)clashWeight+=.62;
          samples++;
        }
        if(samples<20){finish(false);return}
        luminance.sort((a,b)=>a-b);
        const percentile=value=>luminance[Math.min(luminance.length-1,Math.max(0,Math.floor((luminance.length-1)*value)))];
        const tonalRange=percentile(.90)-percentile(.10);
        const averageChroma=chromaTotal/samples;
        const vividShare=vividPixels/samples;
        const paletteClashShare=clashWeight/samples;
        finish(paletteClashShare>=.11||(tonalRange>=.46&&averageChroma>=.16)||(tonalRange>=.34&&vividShare>=.22));
      }catch(error){finish(false)}
    };
    image.src=src;
  });
}
function albumPerimeterContrastOverride(album){
  const identity=`${album?.artist||""} ${album?.title||""}`.toLowerCase();
  if(identity.includes("nas")&&identity.includes("illmatic"))return true;
  if(identity.includes("michael jackson")&&identity.includes("thriller"))return false;
  return null;
}
function applyAlbumPerimeterContrast(popup,source,album){
  if(!popup)return;
  const src=String(source||"").trim();
  popup.dataset.perimeterSource=src;
  popup.classList.remove("albumStrongContrast","albumHarshWarmPerimeter");
  const ambient=albumAmbientStyleVars(album).match(/--album-ambient-rgb:([^;]+)/)?.[1]||"214,171,53";
  popup.style.setProperty("--album-perimeter-rgb",ambient);
  popup.style.setProperty("--album-perimeter-soft-rgb",ambient);
  albumCoverPerimeterPalette(src).then(palette=>{
    if(popup.dataset.perimeterSource!==src||!palette)return;
    popup.style.setProperty("--album-perimeter-rgb",palette.primary);
    popup.style.setProperty("--album-perimeter-soft-rgb",palette.soft);
    popup.classList.toggle("albumHarshWarmPerimeter",Boolean(palette.harshWarm));
  });
  const override=albumPerimeterContrastOverride(album);
  if(override!==null){popup.classList.toggle("albumStrongContrast",override);return}
  albumCoverHasStrongContrast(src).then(strong=>{
    if(popup.dataset.perimeterSource!==src)return;
    popup.classList.toggle("albumStrongContrast",strong);
  });
}
...
window.openAlbumOverviewPopup=function(){
  ...
  applyAlbumPerimeterContrast(popup,albumCoverUrl(album),album);
  popup.classList.remove("hidden");
...
async function adminOverviewRequest(payload){
  ...
  try{
    ...
    if(!res.ok){
      ...
      if(["list_album_review_submissions","list_artist_bio_submissions","list_defining_track_submissions"].includes(payload?.action)){
...
window.saveAlbumOverview=async function(albumId){
  ...
  else clearLocalOverviewOverridesForAlbum(album);
...
window.setAlbumScoreAdmin=async function(albumId){
  ...
  const key=overviewKey(album);
  const previous=albumOverviewRowReadOnly(album);
...
async function persistHeroFocus(albumId,focus){
  ...
  if(!album)return;
  const data=await adminOverviewRequest({...adminAlbumPayload(album,"set_hero_focus"),overview:albumBaseOverview(album),hero_focus:focus});
  if(db&&data&&!data.localOnly)clearLocalOverviewOverridesForAlbum(album);
}
async function persistOverviewFocus(albumId,focus){
  const album=saveAlbumVisualFocus(albumId,"overview_focus",focus);
  if(!album)return;
  const data=await adminOverviewRequest({...adminAlbumPayload(album,"set_overview_focus"),overview:albumBaseOverview(album),overview_focus:focus});
  if(db&&data&&!data.localOnly)clearLocalOverviewOverridesForAlbum(album);
}
async function persistMomentFocus(albumId,focus){
  const album=saveAlbumVisualFocus(albumId,"moment_focus",focus);
  if(!album)return;
  const data=await adminOverviewRequest({...adminAlbumPayload(album,"set_moment_focus"),overview:albumBaseOverview(album),moment_focus:focus});
  if(db&&data&&!data.localOnly)clearLocalOverviewOverridesForAlbum(album);
}
...
const trackRatingSyncQueues=new Map();
window.rateTrack=async function(albumId,trackKeyValue,trackName,value){
  ...
  const previousSync=trackRatingSyncQueues.get(ref)||Promise.resolve();
  const queuedSync=previousSync.catch(()=>{}).then(sync);
  trackRatingSyncQueues.set(ref,queuedSync);
  queuedSync.catch(error=>console.warn("Song rating refresh failed",error)).finally(()=>{if(trackRatingSyncQueues.get(ref)===queuedSync)trackRatingSyncQueues.delete(ref)});
}
...
const PUBLIC_LISTS_STORAGE_KEY="musicaPublicLists";
const PUBLIC_LIST_FOLLOWS_STORAGE_KEY="musicaPublicListFollows";
const PUBLIC_LIST_TYPE_OVERRIDES_KEY="musicaPublicListTypeOverrides";
const seededPublicListDefinitions=[
  {id:"seed-list-summer-breezers",slug:"summer-breezers",title:"Summer Breezers",description:"Warm records for open windows and long evenings.",username:"northsideben",genres:["Soul","Pop","Reggae","R&B"],count:14},
  {id:"seed-list-90s-alternative",slug:"90s-alternative-essentials",title:"90s Alternative Essentials",description:"The records that gave a restless decade its sound.",username:"noahvibes",genres:["Alternative","Rock","Grunge","Indie"],count:20},
  {id:"seed-list-rainy-sunday",slug:"albums-for-a-rainy-sunday",title:"Albums for a Rainy Sunday",description:"Quiet, textured albums for slow afternoons indoors.",username:"quietstormsam",genres:["Folk","Jazz","Soul","Alternative"],count:12},
  {id:"seed-list-2am-records",slug:"2am-records",title:"2AM Records",description:"After-hours listening for when the city goes quiet.",username:"midnightasher",genres:["Electronic","R&B","Trip-Hop","Ambient"],count:16}
];
function localPublicLists(){try{const rows=JSON.parse(localStorage.getItem(PUBLIC_LISTS_STORAGE_KEY)||"[]");return Array.isArray(rows)?rows:[]}catch(error){return []}}
function saveLocalPublicLists(rows){localStorage.setItem(PUBLIC_LISTS_STORAGE_KEY,JSON.stringify(rows||[]))}
function localPublicListFollows(){try{const rows=JSON.parse(localStorage.getItem(PUBLIC_LIST_FOLLOWS_STORAGE_KEY)||"[]");return Array.isArray(rows)?rows.map(String):[]}catch(error){return []}}
function saveLocalPublicListFollows(rows){localStorage.setItem(PUBLIC_LIST_FOLLOWS_STORAGE_KEY,JSON.stringify([...new Set((rows||[]).map(String))]))}
function publicListTypeOverrides(){try{const value=JSON.parse(localStorage.getItem(PUBLIC_LIST_TYPE_OVERRIDES_KEY)||"{}");return value&&typeof value==="object"&&!Array.isArray(value)?value:{}}catch(error){return {}}}
function savePublicListTypeOverride(list,type){const key=String(list?.id||list?.slug||"");if(!key)return;const overrides=publicListTypeOverrides();overrides[key]=publicListCurationType({curation_type:type});localStorage.setItem(PUBLIC_LIST_TYPE_OVERRIDES_KEY,JSON.stringify(overrides))}
function publicListSchemaUnavailable(error){return /schema cache|relation .*lists.*does not exist|could not find .*lists|list_albums|list_follows/i.test(String(error?.message||error||""))}
function publicListSlug(value){return String(value||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"").slice(0,70)||"listener-list"}
function publicListSlugFromPath(){
  const match=String(location.pathname||"").match(/\/list\/([^/?#]+)/i);
  if(!match)return "";
  try{return publicListSlug(decodeURIComponent(match[1]))}catch(error){return publicListSlug(match[1])}
}
let seededPublicListsSource=null;
let seededPublicListsLength=-1;
let seededPublicListsFirst="";
let seededPublicListsLast="";
let seededPublicListsCache=[];
function seededPublicLists(){
  const pool=state.albums&&state.albums.length?state.albums:seedAlbums;
  const first=String(pool[0]?.id||"");
  const last=String(pool[pool.length-1]?.id||"");
  if(seededPublicListsSource===pool&&seededPublicListsLength===pool.length&&seededPublicListsFirst===first&&seededPublicListsLast===last)return seededPublicListsCache;
  const matches=seededPublicListDefinitions.map(()=>[]);
  const seen=seededPublicListDefinitions.map(()=>new Set());
  const scanLimit=Math.min(pool.length,600);
  for(let albumIndex=0;albumIndex<scanLimit;albumIndex++){
    const album=pool[albumIndex];
    if(!album)continue;
    const genre=albumGenreLabel(album).toLowerCase();
    const key=albumIdentityKey(album)||coverKey(album)||String(album.id||"");
    seededPublicListDefinitions.forEach((definition,index)=>{
      if(matches[index].length>=definition.count||!key||seen[index].has(key))return;
      if(definition.genres.some(value=>genre.includes(value.toLowerCase()))){seen[index].add(key);matches[index].push(album)}
    });
    if(matches.every((items,index)=>items.length>=seededPublicListDefinitions[index].count))break;
  }
  seededPublicListsCache=seededPublicListDefinitions.map((definition,index)=>{
    const selected=matches[index];
    for(let attempt=0;selected.length<definition.count&&attempt<Math.min(pool.length*2,definition.count*12);attempt++){
      const album=pool[(index*17+attempt*11)%Math.max(1,pool.length)];
      const key=albumIdentityKey(album)||coverKey(album)||String(album?.id||"");
      if(!album||!key||seen[index].has(key))continue;
      seen[index].add(key);selected.push(album);
    }
    const items=selected.map(albumToLibraryItem);
    return {...definition,user_id:null,seeded:true,is_public:true,featured:true,items,album_count:definition.count,followers_count:5+index*3,created_at:new Date(Date.now()-(index+1)*86400000).toISOString(),updated_at:new Date(Date.now()-(index+1)*3600000).toISOString()};
  }).filter(list=>list.items.length);
  seededPublicListsSource=pool;
  seededPublicListsLength=pool.length;
  seededPublicListsFirst=first;
  seededPublicListsLast=last;
  return seededPublicListsCache;
}
function publicListProfile(list={}){
  const profile=(extras.profileDirectory||[]).find(row=>String(row.user_id||"")&&String(row.user_id)===String(list.user_id||""));
  return profile||{};
}
function normalizedPublicList(list={}){
  const profile=publicListProfile(list);
  const username=String(list.username||profile.username||"listener").replace(/^@/,"");
  const typeOverride=publicListTypeOverrides()[String(list.id||list.slug||"")];
  return {...list,...profile,username,curation_type:publicListCurationType({curation_type:typeOverride||list.curation_type}),items:liveLibraryItems(Array.isArray(list.items)?list.items:[]),album_count:Number(list.album_count||(Array.isArray(list.items)?list.items.length:0)||0),followers_count:Number(list.followers_count||0)};
}
function visiblePublicLists(){
  const rows=[...localPublicLists(),...(extras.publicLists||[]),...convertedSeededPublicLists(),...seededPublicLists()];
  const seen=new Set();
  return rows.map(normalizedPublicList).filter(list=>list.is_public!==false||String(list.user_id||"")===String(loggedInUser()?.id||"")).filter(list=>{
    const key=String(list.id||list.slug||"");
    if(!key||seen.has(key))return false;
    seen.add(key);return true;
  }).sort((a,b)=>Number(b.featured)-Number(a.featured)||Number(b.followers_count)-Number(a.followers_count)||String(b.updated_at||"").localeCompare(String(a.updated_at||"")));
}
async function loadPublicLists(force=false){
  if(extras.publicListsLoaded&&!force)return visiblePublicLists();
  if(extras.publicListsRequest&&!force)return extras.publicListsRequest;
  extras.publicListsRequest=(async()=>{
    if(!db){extras.publicLists=localPublicLists();extras.publicListsLoaded=true;return visiblePublicLists()}
    try{
      const userId=String(loggedInUser()?.id||"");
      let query=db.from("lists").select("id,user_id,title,slug,description,is_public,cover_image_url,featured,curation_type,created_at,updated_at").order("featured",{ascending:false}).order("updated_at",{ascending:false}).limit(60);
      if(!userId)query=query.eq("is_public",true);
      let {data:lists,error:listError}=await query;
      if(listError&&/curation_type|column/i.test(String(listError.message||""))){
        let fallbackQuery=db.from("lists").select("id,user_id,title,slug,description,is_public,cover_image_url,featured,created_at,updated_at").order("featured",{ascending:false}).order("updated_at",{ascending:false}).limit(60);
        if(!userId)fallbackQuery=fallbackQuery.eq("is_public",true);
        const fallbackResult=await fallbackQuery;
        lists=fallbackResult.data;listError=fallbackResult.error;
      }
      if(listError)throw listError;
      const listIds=(lists||[]).map(row=>row.id).filter(Boolean);
      let junctionRows=[],followRows=[];
      if(listIds.length){
        const [junctionResult,followResult]=await Promise.all([
          db.from("list_albums").select("list_id,album_id,position,note").in("list_id",listIds).order("position",{ascending:true}),
          db.from("list_follows").select("list_id,user_id").in("list_id",listIds)
        ]);
        if(junctionResult.error)throw junctionResult.error;
        if(followResult.error)throw followResult.error;
        junctionRows=junctionResult.data||[];followRows=followResult.data||[];
      }
      const albumIds=[...new Set(junctionRows.map(row=>row.album_id).filter(Boolean))];
      let albumRows=[];
      if(albumIds.length){
        const {data,error}=await db.from("album_scores").select("*").in("id",albumIds);
        if(error)throw error;
        albumRows=data||[];
      }
      const albumMap=new Map(albumRows.map(album=>[String(album.id),album]));
      extras.listFollows=followRows;
      extras.publicLists=(lists||[]).map(list=>{
        const items=junctionRows.filter(row=>String(row.list_id)===String(list.id)).sort((a,b)=>Number(a.position)-Number(b.position)).map(row=>albumMap.get(String(row.album_id))).filter(Boolean).map(albumToLibraryItem);
        return {...list,items,album_count:items.length,followers_count:followRows.filter(row=>String(row.list_id)===String(list.id)).length};
      });
      extras.publicListsLoaded=true;
    }catch(error){
      if(!publicListSchemaUnavailable(error))console.warn("Unable to load public lists",error?.message||error);
      extras.publicLists=localPublicLists();extras.publicListsLoaded=true;
    }
    return visiblePublicLists();
  })();
  try{return await extras.publicListsRequest}finally{extras.publicListsRequest=null}
}
function publicListBySlug(slug){
  const list=visiblePublicLists().find(item=>publicListSlug(item.slug||item.title)===publicListSlug(slug))||null;
  if(list?.curation_type==="list"&&seededLibraryIndexFor(list)>=0)return seededLibraryAsPublicList(fullSeededLibraryByKey(list.id)||list);
  return list;
}
function publicListCurationType(list={}){return String(list.curation_type||"list").toLowerCase()==="library"?"library":"list"}
function publicListOwnedByUser(list){return Boolean(list)&&!list.seeded&&((list.local_owner_device_id&&list.local_owner_device_id===state.deviceId)||(list.user_id&&String(list.user_id)===String(loggedInUser()?.id||"")))}
const seededLibraryBlueprints=[
  ["miraafterhours","Mira's After Hours Shelf",["Soul","R&B","Jazz","Hip-Hop","Pop"],9],
  ["northsideben","Ben's Northside Rotation",["Hip-Hop","Alternative","Rock","Electronic"],14],
  ["velvetnoah","Noah's Velvet Records",["Classic Rock","Soul","Folk Rock","Pop Rock"],18],
  ["cassetteivy","Ivy's Cassette Stack",["Alternative","Indie","Pop","Electronic","Rock"],11],
  ["lowlightmaya","Maya's Low Light Library",["Trip-Hop","Soul","Jazz","Electronic","R&B"],7],
  ["goldenleo","Leo's Golden Hour Picks",["Pop Rock","Soul","Folk","Classic Rock"],22],
  ["basementtess","Tess's Basement Finds",["Punk","Alternative Rock","Hip-Hop","Metal"],13],
  ["julesonwax","Jules on Wax",["Jazz","Soul","Funk","Hip-Hop"],16],
  ["midnightasher","Asher's Midnight Albums",["Electronic","Alternative","R&B","Ambient"],6],
  ["sadiecrate","Sadie's Crate Notes",["Folk Rock","Country","Soul","Rock"],20],
  ["orbiteli","Eli's Orbit Library",["Electronic","Pop","Alternative","Hip-Hop"],10],
  ["staticnina","Nina's Static Favorites",["Metal","Industrial","Alternative Rock","Electronic"],15],
  ["sundownomar","Omar's Sundown Shelf",["Reggae","Soul","Hip-Hop","Rock"],8],
  ["violetmax","Max's Violet Rotation",["Glam Rock","Pop","Alternative","Classic Rock"],24],
  ["riverana","Ana's River Records",["Folk","Singer-Songwriter","Soul","Latin"],12],
  ["latecheckout","Late Checkout Library",["R&B","Pop","Hip-Hop","Electronic"],19],
  ["secondpress","Second Press Picks",["Classic Rock","Psychedelic Rock","Jazz","Soul"],17],
  ["opalren","Ren's Opal Shelf",["Art Rock","Alternative","Electronic","Pop"],9],
  ["bricklanejo","Jo's Brick Lane Library",["Punk Rock","New Wave","Hip-Hop","Dance"],14],
  ["quietstormsam","Sam's Quiet Storm",["Soul","R&B","Jazz","Pop"],6],
  ["hazelneedle","Hazel's Needle Drops",["Rock","Folk Rock","Blues","Soul"],21],
  ["monoelle","Elle's Mono Favorites",["Pop","Electronic","Disco","Alternative"],10],
  ["coppermiles","Miles's Copper Shelf",["Jazz","Hip-Hop","Soul","Funk"],18],
  ["nightbuslena","Lena's Night Bus Albums",["Alternative","Electronic","Trip-Hop","Rock"],13],
  ["gardenari","Ari's Garden Records",["Folk","Pop Rock","Soul","Psychedelic"],8],
  ["blackstarvic","Vic's Black Star Stack",["Hip-Hop","Soul","Jazz","Alternative"],23],
  ["sunroomkai","Kai's Sunroom Library",["Pop","Indie Pop","Folk Rock","R&B"],12],
  ["heavylila","Lila's Heavy Shelf",["Metal","Hard Rock","Punk","Alternative"],16],
  ["bluelinetom","Tom's Blue Line Picks",["Rock","Soul","Reggae","Hip-Hop"],7],
  ["archivemina","Mina's Archive Finds",["Classical","Jazz","Electronic","Art Rock"],20],
  ["weekendnash","Nash's Weekend Rotation",["Hip-Hop","Pop","R&B","Dance"],11],
  ["sparrowels","Elsie's Sparrow Shelf",["Singer-Songwriter","Folk","Country","Soul"],15],
  ["cityhaze","City Haze Library",["Alternative Rock","Hip-Hop","Electronic","Punk"],19],
  ["marblezara","Zara's Marble Records",["Soul","Disco","Pop","Jazz"],9],
  ["cassianwave","Cassian's Wave Shelf",["New Wave","Post-Punk","Electronic","Pop"],17],
  ["analogreece","Reece's Analog Library",["Classic Rock","Blues Rock","Folk Rock","Soul"],22],
  ["smallhoursnia","Nia's Small Hours",["Ambient","R&B","Electronic","Jazz"],6],
  ["festivaldrew","Drew's Festival Picks",["Rock","Hip-Hop","Pop","Electronic","Soul"],24],
  ["inkandvinyl","Ink & Vinyl Shelf",["Punk","Metal","Hip-Hop","Alternative"],13],
  ["pearlrowan","Rowan's Pearl Rotation",["Pop Rock","Soul","Folk","Alternative","Jazz"],18]
];
const SEEDED_LIBRARY_OVERRIDES_KEY="musicaSeededLibraryIdentityOverrides";
function seededLibraryIdentityOverrides(){
  try{
    const value=JSON.parse(localStorage.getItem(SEEDED_LIBRARY_OVERRIDES_KEY)||"{}");
    return value&&typeof value==="object"&&!Array.isArray(value)?value:{};
  }catch(error){return {}}
}
function seededLibraryKey(index){return "seed-library-"+String(index+1).padStart(2,"0")}
function seededLibraryBlueprint(index){
  const blueprint=seededLibraryBlueprints[index];
  if(!blueprint)return null;
  const [defaultUsername,defaultTitle,genres,count]=blueprint;
  const override=seededLibraryIdentityOverrides()[seededLibraryKey(index)]||{};
  const type=String(override.type||"library").toLowerCase()==="list"?"list":"library";
  return [String(override.username||defaultUsername),String(override.title||defaultTitle),genres,count,defaultUsername,type];
}
function seededLibraryCurationType(index){return seededLibraryBlueprint(Number(index))?.[5]||"library"}
function seededLibraryIndexFor(library={}){
  const idMatch=String(library.id||"").match(/^seed-library-(\d+)$/);
  if(idMatch)return Number(idMatch[1])-1;
  const deviceId=String(library.device_id||"");
  if(deviceId.startsWith("seed-device-")){
    const stableUsername=deviceId.slice("seed-device-".length).toLowerCase();
    return seededLibraryBlueprints.findIndex(([username])=>String(username).toLowerCase()===stableUsername);
  }
  return -1;
}
function isSeededLibrary(library={}){
  return Boolean(library.seeded||String(library.id||"").startsWith("seed-library-")||String(library.device_id||"").startsWith("seed-device-"));
}
function seededHash(value){
  return String(value||"").split("").reduce((sum,char,index)=>sum+char.charCodeAt(0)*(index+17),0);
}
function seededAlbumGenreMatch(album,genre){
  const haystack=[albumGenreLabel(album),album.genre,album.tag,album.summary,album.title,album.artist].join(" ").toLowerCase();
  return String(genre||"").toLowerCase().split(/\s+/).some(part=>part.length>2&&haystack.includes(part));
}
let seededLibraryCacheKey="";
let seededLibraryCache=[];
let uniqueSeedAlbumPoolSource=null;
let uniqueSeedAlbumPoolLength=-1;
let uniqueSeedAlbumPoolFirst="";
let uniqueSeedAlbumPoolLast="";
let uniqueSeedAlbumPoolCache=[];
function seededAlbumKey(album){
  return [album?.title,album?.artist].map(value=>String(value||"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim()).join("::");
}
function uniqueSeedAlbumPool(){
  const source=state.albums&&state.albums.length?state.albums:seedAlbums;
  const first=String(source[0]?.id||seededAlbumKey(source[0]));
  const last=String(source[source.length-1]?.id||seededAlbumKey(source[source.length-1]));
  if(uniqueSeedAlbumPoolSource===source&&uniqueSeedAlbumPoolLength===source.length&&uniqueSeedAlbumPoolFirst===first&&uniqueSeedAlbumPoolLast===last)return uniqueSeedAlbumPoolCache;
  const rows=source.filter(album=>album&&album.title&&album.artist);
  const seen=new Set();
  uniqueSeedAlbumPoolCache=rows.reduce((items,album)=>{
    const key=seededAlbumKey(album);
    if(key&&seen.has(key))return items;
    if(key)seen.add(key);
    items.push(album);
    return items;
  },[]);
  uniqueSeedAlbumPoolSource=source;
  uniqueSeedAlbumPoolLength=source.length;
  uniqueSeedAlbumPoolFirst=first;
  uniqueSeedAlbumPoolLast=last;
  return uniqueSeedAlbumPoolCache;
}
function seededLibraryItems(username,genres,count,pool){
  pool=Array.isArray(pool)?pool:uniqueSeedAlbumPool();
  if(!pool.length)return [];
  const offset=seededHash(username)%pool.length;
  const chosen=[];
  const chosenKeys=new Set();
  const addAlbum=album=>{
    const key=seededAlbumKey(album);
    if(album&&key&&!chosenKeys.has(key)){chosenKeys.add(key);chosen.push(album)}
  };
  genres.forEach((genre,index)=>{
    const matches=pool.filter(album=>seededAlbumGenreMatch(album,genre));
    addAlbum(matches[(offset+index*3)%Math.max(1,matches.length)]||pool[(offset+index*5)%pool.length]);
  });
  for(let step=0;chosen.length<count&&step<pool.length*3;step++){
    const album=pool[(offset+step*7+genres.length)%pool.length];
    addAlbum(album);
  }
  return chosen.slice(0,count).map(albumToLibraryItem);
}
function seededCommunityLibraries(limit=seededLibraryBlueprints.length){
  const pool=uniqueSeedAlbumPool();
  limit=Math.max(0,Math.min(Number(limit)||seededLibraryBlueprints.length,seededLibraryBlueprints.length));
  const cacheKey=[pool.length,pool[0]?.id,pool[pool.length-1]?.id,limit,JSON.stringify(seededLibraryIdentityOverrides())].join("::");
  if(seededLibraryCacheKey===cacheKey&&seededLibraryCache.length)return seededLibraryCache;
  seededLibraryCacheKey=cacheKey;
  seededLibraryCache=seededLibraryBlueprints.slice(0,limit).map((blueprint,index)=>{
    const [username,title,genres,count,stableUsername,type]=seededLibraryBlueprint(index);
    const items=seededLibraryItems(stableUsername,genres,count,pool);
    return {
      id:seededLibraryKey(index),
      device_id:"seed-device-"+stableUsername,
      username,
      title,
      seeded:true,
      source:"muze_editorial_seed",
      curation_type:type,
      items,
      album_count:items.length,
      followers_count:(seededHash(stableUsername)%9)+Math.floor(index/4),
      updated_at:new Date(Date.now()-(index+2)*36e5).toISOString()
    };
  }).filter(library=>library.items.length);
  return seededLibraryCache;
}
let staticSeededLibraryCache={};
function staticSeededCommunityLibraries(limit=6){
  limit=Math.max(0,Math.min(Number(limit)||6,seededLibraryBlueprints.length));
  const pool=state.albums&&state.albums.length?state.albums:seedAlbums;
  const cacheKey=[limit,pool.length,pool[0]?.id||"",pool[pool.length-1]?.id||"",JSON.stringify(seededLibraryIdentityOverrides())].join("::");
  if(staticSeededLibraryCache[cacheKey])return staticSeededLibraryCache[cacheKey];
  const usedPreviewAlbums=new Set();
  const albumKey=album=>seededAlbumKey(album)||String(album?.id||album?.cover_url||"");
  const libraries=seededLibraryBlueprints.slice(0,limit).map((blueprint,index)=>{
    const [username,title,genres,count,stableUsername,type]=seededLibraryBlueprint(index);
    const offset=pool.length?seededHash(stableUsername)%pool.length:0;
    const items=[];
    if(pool.length){
      const step=11+(index%7)*4;
      for(let attempt=0;items.length<Math.min(count,4)&&attempt<pool.length*2;attempt++){
        const album=pool[(offset+index*17+attempt*step)%pool.length];
        const key=albumKey(album);
        if(!album||!key)continue;
        if(usedPreviewAlbums.has(key)&&attempt<pool.length)continue;
        usedPreviewAlbums.add(key);
  ...
  staticSeededLibraryCache[cacheKey]=libraries;
  return staticSeededLibraryCache[cacheKey];
}
function seededLibraryAsPublicList(library){
  if(!library)return null;
  const genres=(library.genres||[]).filter(Boolean);
  return {...library,curation_type:"list",slug:publicListSlug(library.id||library.title),description:`A listener-curated mix of ${genres.slice(0,3).join(", ")||"Muze favorites"}.`,is_public:true,featured:true};
}
function convertedSeededPublicLists(){
  const listIndexes=Object.entries(seededLibraryIdentityOverrides())
    .filter(([,override])=>String(override?.type||"").toLowerCase()==="list")
    .map(([key])=>Number(String(key).match(/^seed-library-(\d+)$/)?.[1]||0)-1)
    .filter(index=>index>=0&&index<seededLibraryBlueprints.length);
  if(!listIndexes.length)return [];
  const previews=staticSeededCommunityLibraries(Math.max(...listIndexes)+1);
  const wanted=new Set(listIndexes);
  return previews.filter(library=>wanted.has(seededLibraryIndexFor(library))).map(seededLibraryAsPublicList);
}
function fullSeededLibraryByKey(key){
  const match=String(key||"").match(/^seed-library-(\d+)$/);
  if(!match)return null;
  const index=Number(match[1])-1;
  const blueprint=seededLibraryBlueprint(index);
  if(!blueprint)return null;
  const [username,title,genres,count,stableUsername,type]=blueprint;
  const items=seededLibraryItems(stableUsername,genres,count,uniqueSeedAlbumPool());
  return {
    id:seededLibraryKey(index),
    device_id:"seed-device-"+stableUsername,
    username,
...
window.editSeededLibraryIdentity=async function(index){
  if(!isAdminUnlocked()){alert("Unlock admin mode first.");return}
  index=Number(index);
  const current=seededLibraryBlueprint(index);
  if(!current)return;
  const [currentUsername,currentTitle,genres,count,stableUsername,currentType]=current;
  const typeAnswer=prompt("Choose how this seeded curation appears (library or list):",currentType);
  if(typeAnswer===null)return;
  const type=String(typeAnswer).trim().toLowerCase().replace(/s$/,"");
  if(type!=="library"&&type!=="list"){alert("Enter either library or list.");return}
  const titleAnswer=prompt(`Edit this ${type} title:`,currentTitle);
  if(titleAnswer===null)return;
  const title=String(titleAnswer).trim().slice(0,70);
  if(!title){alert("The title cannot be empty.");return}
  const usernameAnswer=prompt("Edit the username (without @):",currentUsername);
  if(usernameAnswer===null)return;
  const username=String(usernameAnswer).trim().replace(/^@+/,"").slice(0,32);
  if(!/^[a-zA-Z0-9._-]{2,32}$/.test(username)){
    alert("Use 2-32 letters, numbers, dots, underscores, or hyphens for the username.");
    return;
  }
  const duplicate=seededLibraryBlueprints.some((_,otherIndex)=>otherIndex!==index&&String(seededLibraryBlueprint(otherIndex)?.[0]||"").toLowerCase()===username.toLowerCase());
  if(duplicate){alert("That seeded-library username is already in use.");return}

  const key=seededLibraryKey(index);
  const overrides=seededLibraryIdentityOverrides();
  overrides[key]={...(overrides[key]||{}),title,username,type};
  localStorage.setItem(SEEDED_LIBRARY_OVERRIDES_KEY,JSON.stringify(overrides));
  seededLibraryCacheKey="";
  seededLibraryCache=[];
  staticSeededLibraryCache={};

  const deviceId="seed-device-"+stableUsername;
  const now=new Date().toISOString();
  const existing=(extras.libraries||[]).find(library=>String(library.device_id||"")===deviceId);
  const localRows=localLibraries().filter(library=>String(library.device_id||"")!==deviceId);
  const generated=type==="library"?fullSeededLibraryByKey(key):null;
  if(generated){
    const localRow={...generated,id:key,device_id:deviceId,username,title,curation_type:type,updated_at:now};
    extras.libraries=existing
      ?extras.libraries.map(library=>String(library.device_id||"")===deviceId?{...library,username,title,items:generated.items,album_count:count,seeded:true,curation_type:type,updated_at:now}:library)
      :[localRow,...(extras.libraries||[])];
    saveLocalLibraries([localRow,...localRows]);
  }else{
    extras.libraries=(extras.libraries||[]).filter(library=>String(library.device_id||"")!==deviceId);
    saveLocalLibraries(localRows);
  }
  librariesView();

  if(type==="library"&&generated&&db&&loggedInUser()){
    const payload={device_id:deviceId,user_id:loggedInUser().id,username,title,items:generated.items,album_count:count,updated_at:now};
    const result=existing?.id&&!String(existing.id).startsWith("seed-library-")
      ?await db.from("user_libraries").update(payload).eq("id",existing.id)
      :await db.from("user_libraries").upsert(payload,{onConflict:"device_id"});
    if(result.error){
      console.warn("Seeded library identity could not be published",result.error.message||result.error);
      alert("The title and username were saved in this browser, but could not be published: "+result.error.message);
      return;
    }
    await loadLibraries().catch(error=>console.warn("Unable to refresh edited seeded library",error));
    if(state.view==="libraries")librariesView();
  }
};
function sortedLibraryItems(items){return items.slice()}
function liveLibraryItem(item){
  const album=fastAlbumMatch(item);
...
function librarySimilarity(items,mineOverride){
  const mine=Array.isArray(mineOverride)?mineOverride:ownComparableLibraryItems();
...
function albumPublicAppearanceCounts(album){
  const seen=new Set();
  let libraries=0;
  let suggestions=0;
...
function albumYearAccolade(album){
  const row=albumOverviewRowReadOnly(album);
  const rank=Number(album?.year_accolade_rank??row?.year_accolade_rank);
  const year=String(album?.year_accolade_year??row?.year_accolade_year??"").match(/\b(?:19|20)\d{2}\b/)?.[0]||"";
  const source=String(album?.year_accolade_source??row?.year_accolade_source??"").trim();
  const label=String(album?.year_accolade_label??row?.year_accolade_label??"").trim();
  return Number.isInteger(rank)&&rank>0&&year&&source?{rank,year,source,label}:null;
}
function albumMemberCollectionRate(album){
  const members=new Map();
  const profileKeysByUsername=new Map();
  (extras.profileDirectory||[]).forEach((profile,index)=>{
    const username=String(profile?.username||"").replace(/^@/,"").trim().toLowerCase();
    const key=String(profile?.user_id?`user:${profile.user_id}`:username?`username:${username}`:`profile:${index}`);
    members.set(key,{added:false});
    if(username)profileKeysByUsername.set(username,key);
  });
  const sourceOwnerKey=(source,index)=>{
    const username=String(source?.username||source?.profile_username||source?.name||"").replace(/^@/,"").trim().toLowerCase();
    return String(source?.user_id?`user:${source.user_id}`:username&&profileKeysByUsername.has(username)?profileKeysByUsername.get(username):username?`username:${username}`:source?.local_owner_device_id?`device:${source.local_owner_device_id}`:source?.device_id?`device:${source.device_id}`:source?.owner_id?`owner:${source.owner_id}`:source?.id?`source:${source.id}`:`source:${index}`);
  };
  const addSource=(source,index)=>{
    if(!source||source.seeded||String(source.id||"").startsWith("seed-"))return;
    const items=Array.isArray(source.items)?source.items:[];
    const key=sourceOwnerKey(source,index);
    const current=members.get(key)||{added:false};
    current.added=current.added||hasMatchingAlbum(items,album);
    members.set(key,current);
  };
  (extras.libraries||[]).forEach((library,index)=>addSource(library,`library-${index}`));
  [...localPublicLists(),...(extras.publicLists||[])].forEach((list,index)=>addSource(list,`suggestion-${index}`));
  const ownItems=myLibraryItems();
  if(ownItems.length)addSource({items:ownItems,user_id:loggedInUser()?.id,device_id:state.deviceId,username:profileLibraryUsername()||currentUsername()},"mine");
  const total=members.size;
  const added=[...members.values()].filter(member=>member.added).length;
  return {total,added,percentage:total?Math.round((added/total)*100):0};
}
function albumExceptionalFeatherIcon(mirrored=false){
  return `<svg class="exceptionalFeather${mirrored?" exceptionalFeatherRight":""}" viewBox="0 0 10 20" aria-hidden="true"><path d="M7.8 1.1C3.2 4.6.5 10.3 7.9 18.9M6.4 3.1C5 2.7 4 3.1 3.2 4.1M4.4 5.9C3 5.6 2 6.2 1.4 7.3M3.1 8.9C1.8 8.9.9 9.7.6 10.9M3.2 12.1c-1.3.3-2 1.1-2 2.3M4.8 15.2c-1.1.5-1.6 1.4-1.4 2.5"/></svg>`;
}
function albumStatIcon(type){
  if(type==="rank")return `<svg class="albumStatIcon" viewBox="0 0 20 20" aria-hidden="true"><path d="m10 1.8 2.35 4.75 5.25.76-3.8 3.7.9 5.23L10 13.77l-4.7 2.47.9-5.23-3.8-3.7 5.25-.76L10 1.8Z"/></svg>`;
  return `<svg class="albumStatIcon" viewBox="0 0 22 18" aria-hidden="true"><circle cx="7" cy="5" r="3"/><circle cx="15.5" cy="5.5" r="2.6"/><path d="M1.5 16c.2-4.2 2.1-6.3 5.7-6.3 3.7 0 5.6 2.1 5.8 6.3H1.5Zm10.2-.2c-.1-2-.7-3.7-1.9-4.8 1-.8 2.2-1.2 3.8-1.2 3.7 0 5.7 2 5.9 6h-7.8Z"/></svg>`;
}
...
function quickCurrentLibraryCard(){
  const published=matchingPublishedLibrary();
  if(published){
    const publishedItems=liveLibraryItems(Array.isArray(published.items)?published.items:[]);
    if(publishedItems.length){
      return {
        ...normalizeOwnLibrary(published),
        items:publishedItems,
        album_count:publishedItems.length,
        followers_count:Number(published.followers_count||0),
        isMine:true
      };
    }
  }
  const rawItems=myLibraryItems();
  const items=liveLibraryItems(rawItems.slice(-6));
  const username=profileLibraryUsername()||"Listener";
  return {
    id:"local-library-"+state.deviceId,
    device_id:state.deviceId,
    username,
    title:`${username}'s Library`,
    items,
    album_count:rawItems.length,
    followers_count:0,
    isMine:true,
    ...profileAvatarFields()
  };
}
function refreshOwnLibraryHero(){
  if(state.view!=="libraries")return;
  const current=document.querySelector("body.libraryView .mockYourLibrary");
  const library=currentLibraryCard();
  if(!current||!library)return;
  const holder=document.createElement("div");
  holder.innerHTML=ownLibraryHero(library);
  const replacement=holder.firstElementChild;
  if(replacement)current.replaceWith(replacement);
}
function visibleLibraries(seedLimit=seededLibraryBlueprints.length,includeMine=true){
  const mine=includeMine?currentLibraryCard():null;
  const userId=String(loggedInUser()?.id||state.userProfile?.user_id||"");
  const seenLibraries=new Set();
  const realSource=includeMine?(extras.libraries||[]):(extras.libraries||[]).slice(0,Math.max(3,seedLimit));
  const source=[...realSource,...seededCommunityLibraries(seedLimit)].reduce((libraries,library)=>{
    if(!library)return libraries;
    const key=String(library.id||library.device_id||library.username||"");
    if(!key||seenLibraries.has(key))return libraries;
    seenLibraries.add(key);
    libraries.push(library);
    return libraries;
  },[]);
  const others=source.filter(l=>{
    const seedIndex=seededLibraryIndexFor(l);
    if(l?.curation_type==="list"||(seedIndex>=0&&seededLibraryCurationType(seedIndex)==="list"))return false;
    return (!mine||String(l.id)!==String(mine.id))&&String(l.device_id||"")!==String(state.deviceId)&&!(userId&&String(l.user_id||"")===userId);
  });
  return mine?[mine,...others]:source;
}
...
async function loadProfileDirectory(){
  const profileDb=publicDataDb||db;
  extras.profileDirectoryLoaded=false;
  if(profileDb){
    const pageSize=1000;
    const rows=[];
    let error=null;
    for(let from=0;;from+=pageSize){
      const result=await profileDb.from("public_user_profiles").select("user_id,username,avatar_url,avatar_config,avatar_svg,avatar_type,created_at").range(from,from+pageSize-1);
      if(result.error){error=result.error;break}
      const batch=result.data||[];
      rows.push(...batch);
      if(batch.length<pageSize)break;
    }
    if(!error){extras.profileDirectory=rows;extras.profileDirectoryLoaded=true;return extras.profileDirectory}
    console.warn("Unable to load complete public profile directory",error.message||error);
  }
...
async function loadLibraries(){
  ...
  loadPublicLists().then(()=>{if(state.view==="libraries")librariesView()}).catch(error=>console.warn("Unable to refresh public lists",error));
  if(db){
    try{
      const {data,error}=await Promise.race([
        db.from("library_feed").select("*").order("updated_at",{ascending:false}).limit(50),
        new Promise(resolve=>setTimeout(()=>resolve({data:null,error:new Error("Library feed timed out")}),2500))
      ]);
      if(!error){
        extras.libraries=data||[];
        const published=matchingPublishedLibrary();
        const publishedItems=liveLibraryItems(Array.isArray(published?.items)?published.items:[]);
        if(publishedItems.length>myLibraryItems().length)saveMyLibraryItems(publishedItems);
        refreshOwnLibraryHero();
        await Promise.race([loadLibraryFollows(),new Promise(resolve=>setTimeout(resolve,1000))]);
        renderNotifications();
        return extras.libraries;
      }
      console.warn("Unable to load library feed",error.message||error);
    }catch(error){
      console.warn("Unable to load library feed",error.message||error);
    }
  }
...
function libraryProfileMetaMarkup(stats={},albumCount=0){
  const albumsRated=Number(stats.albumsRated||0);
  const songsRated=Number(stats.songsRated||0);
  const songsShared=Number(stats.songsShared||0);
  const commentsLeft=Number(stats.commentsLeft||0);
  const items=[
    ["albumsAdded","&#9673;",`${albumCount} album${albumCount===1?"":"s"}`],
    ["albumsRated","&#9734;",albumsRated?`${albumsRated} album${albumsRated===1?"":"s"} rated`:"No public album ratings"],
    ["songsRated","&#9835;",songsRated?`${songsRated} song${songsRated===1?"":"s"} rated`:"No public song ratings"],
    ["songsShared","&#8599;",songsShared?`${songsShared} item${songsShared===1?"":"s"} shared`:"Not shared"],
    ["commentsLeft","&#9675;",commentsLeft?`${commentsLeft} comment${commentsLeft===1?"":"s"}`:"No comments"]
  ];
  return items.map(([key,icon,label])=>`<button type="button" class="libraryMetaItem" onclick="openProfileActivity('${key}')"><span aria-hidden="true">${icon}</span><small>${escapeHtml(label)}</small></button>`).join("");
}
function openLibraryDetails(library){
  ...
  $("#albumModalContent").innerHTML='<div class="libraryDetailBrand"><img src="/Muze%20correct%20logo%20dark-word.png?v=dark-word-v1" alt="Muze"></div><div class="sectionTitle libraryDetailTitle"><div><h2>'+escapeHtml(library.title||"Library")+'</h2><span class="muted">@'+escapeHtml(library.username||"Listener")+' &middot; '+albumCount+' album'+(albumCount===1?'':'s')+'</span></div><div class="libraryDetailActions">'+addAction+'</div></div><div id="libraryProfileStatsGrid" class="libraryMetaStrip">'+libraryProfileMetaMarkup(baseStats,albumCount)+'</div><div class="grid libraryFullGrid">'+(items.map(item=>libraryAlbumCard(item,isMine,isMine)).join("")||'<div class="empty">No albums yet.</div>')+'</div><div class="libraryDetailFlourish" aria-hidden="true"><span></span><i></i><span></span></div>';
  $("#albumModal").classList.remove("hidden");
  loadPublicProfileStats(library).then(stats=>{
    const grid=$("#libraryProfileStatsGrid");
    if(grid&&stats)grid.innerHTML=libraryProfileMetaMarkup({...baseStats,...stats,albumsAdded:Math.max(Number(stats.albumsAdded||0),baseStats.albumsAdded)},albumCount);
  }).catch(error=>console.warn("Unable to load public profile stats",error.message||error));
}function fastCommunityLibraries(limit=3,username=currentUsername()){
  const normalizedUsername=String(username||"").toLowerCase();
  const seen=new Set();
  const rows=[];
  const add=library=>{
    if(!library)return;
    const seedIndex=seededLibraryIndexFor(library);
    if(library.curation_type==="list"||(seedIndex>=0&&seededLibraryCurationType(seedIndex)==="list"))return;
    if(library.device_id===state.deviceId||library.isMine)return;
    if(normalizedUsername&&String(library.username||"").toLowerCase()===normalizedUsername)return;
    const key=String(library.id||library.device_id||library.username||library.title||"");
    const identity=String(library.username||library.title||key).toLowerCase();
    const stableIdentity=String(library.device_id||"").toLowerCase();
    if(!key||seen.has(key)||seen.has(identity)||(stableIdentity&&seen.has(stableIdentity)))return;
    seen.add(key);
    seen.add(identity);
    if(stableIdentity)seen.add(stableIdentity);
    rows.push(library);
  };
  (extras.libraries||[]).forEach(add);
  staticSeededCommunityLibraries(limit).forEach(add);
  return rows.slice(0,limit);
}
function openLibraryDetailsById(key){
  const library=fullSeededLibraryByKey(key)||[
    quickCurrentLibraryCard(),
    ...(extras.libraries||[]),
    ...staticSeededCommunityLibraries(seededLibraryBlueprints.length)
  ].find(l=>String(l.id)===String(key)||String(l.device_id)===String(key));
...
function libraryPreviewBlock(library){
  ...
  const rawItems=allItems.slice(0,4);
  const key=escapeJsString(library.id||library.device_id||"");
  const followers=Number(library.followers_count||0);
  const albumCount=Number(library.album_count||rawItems.length||0);
  const tags=[...new Set(rawItems.map(item=>String(item.genre||albumGenreLabel(item)||"").trim()).filter(g=>g&&g!=="Album"))].slice(0,3);
  const isSeeded=isSeededLibrary(library);
  const seedIndex=seededLibraryIndexFor(library);
  const canFollow=!isSeeded&&library.id&&String(library.id).indexOf("local-library-")!==0;
  const followAction=canFollow?"followLibrary('"+escapeJsString(library.id)+"')":"openLibraryDetailsById('"+key+"')";
  const adminIdentityEdit=isAdminUnlocked()&&seedIndex>=0?'<button class="libraryAdminIdentityBtn" title="Edit title, username, and Library/Suggestion type" onclick="event.stopPropagation();editSeededLibraryIdentity('+seedIndex+')">Edit</button>':'';
  const preview=rawItems.slice(0,3).map((item,index)=>{
    const image=String(item.cover_url||"").trim();
    const title=String(item.title||"Album").trim();
    return '<article class="mockAlbumTile '+(index===0?'mockFeaturedAlbum':'mockSideAlbum')+'" onclick="event.stopPropagation();openLibraryAlbum(\''+encodeURIComponent(JSON.stringify(item))+'\')"><div class="mockAlbumCover">'+(image?'<img src="'+escapeHtml(image)+'" alt="'+escapeHtml(title)+'">':'<strong>'+escapeHtml(title.slice(0,1)||"M")+'</strong>')+'</div></article>';
  }).join("");
  const extraCount=Math.max(0,albumCount-3);
  return '<div class="libraryCard mockLibraryCard communityShelfCard curatedLibraryCard" role="button" tabindex="0" onclick="openLibraryDetailsById(\''+key+'\')" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();openLibraryDetailsById(\''+key+'\')}"><div class="mockLibraryTop"><div class="mockAvatar">'+lightweightLibraryAvatar(library)+'</div><div class="mockLibraryTitle"><span class="curatedTypeBadge libraryTypeBadge">Library</span><h3>'+escapeHtml(library.title||"Library")+'</h3><p class="mockLibraryByline"><span class="mockUsername">@'+escapeHtml(library.username||"Listener")+'</span><span class="mockAlbumMeta"> &middot; '+libraryAlbumCountText(albumCount)+'</span><span class="mockFollowerMeta"> &middot; '+followers+' follower'+(followers===1?'':'s')+'</span></p></div></div><div class="mockAlbumPreview '+(rawItems.length>=3?'hasMobileTrio':'')+'">'+(preview||'<div class="emptyMini">No public albums yet.</div>')+(extraCount?'<div class="mockMoreTile">+'+extraCount+'</div>':'')+'</div><div class="mockLibraryActions"><button class="libraryExploreBtn" onclick="event.stopPropagation();openLibraryDetailsById(\''+key+'\')">Explore &rarr;</button><span class="libraryAdminIdentitySlot">'+adminIdentityEdit+'<button class="libraryFollowBtn" onclick="event.stopPropagation();'+followAction+'">Follow</button></span></div></div>';
}
function publicListPreviewBlock(rawList){
  const list=normalizedPublicList(rawList);
  const curationType=publicListCurationType(list);
  const isLibrary=curationType==="library";
  const items=list.items.slice(0,3);
  const followers=Number(list.followers_count||0);
  const albumCount=Number(list.album_count||list.items.length||0);
  const slug=escapeJsString(list.slug||publicListSlug(list.title));
  const preview=items.map(item=>{
    const image=String(item.cover_url||"").trim();
    return '<article class="mockAlbumTile" onclick="event.stopPropagation();openLibraryAlbum(\''+encodeURIComponent(JSON.stringify(item))+'\')"><div class="mockAlbumCover">'+(image?'<img src="'+escapeHtml(image)+'" alt="'+escapeHtml(item.title||"Album cover")+'">':'<strong>'+escapeHtml(String(item.title||"M").slice(0,1))+'</strong>')+'</div></article>';
  }).join("");
  const extraCount=Math.max(0,albumCount-3);
  ...
  const ownerActions=(publicListOwnedByUser(list)||isAdminUnlocked())?'<button class="libraryAdminIdentityBtn" onclick="event.stopPropagation();'+editAction+'">Edit</button>':'';
  const description=String(list.description||(isLibrary?"A listener's personal album library.":"A listener-curated album suggestion.")).trim();
  return '<div class="libraryCard mockLibraryCard communityShelfCard curatedListCard '+(isLibrary?'curatedLibraryCard':'')+'" role="button" tabindex="0" onclick="openPublicList(\''+slug+'\')" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();openPublicList(\''+slug+'\')}"><div class="mockLibraryTop"><div class="mockAvatar">'+lightweightLibraryAvatar(list)+'</div><div class="mockLibraryTitle"><span class="curatedTypeBadge '+(isLibrary?'libraryTypeBadge':'listTypeBadge')+'">'+(isLibrary?'Library':'Suggestion')+'</span><h3>'+escapeHtml(list.title||(isLibrary?"Listener library":"Listener suggestion"))+'</h3><p class="mockLibraryByline"><span class="mockUsername">'+(isLibrary?'@':'by @')+escapeHtml(list.username||"listener")+'</span><span class="mockAlbumMeta"> &middot; '+libraryAlbumCountText(albumCount)+'</span><span class="mockFollowerMeta"> &middot; '+followers+' follower'+(followers===1?'':'s')+'</span></p></div></div>'+(isLibrary?'':'<div class="mockDescriptor curatedListDescription">'+escapeHtml(description)+'</div>')+'<div class="mockAlbumPreview '+(items.length>=3?'hasMobileTrio':'')+'">'+(preview||'<div class="emptyMini">No albums yet.</div>')+(extraCount?'<div class="mockMoreTile">+'+extraCount+'</div>':'')+'</div><div class="mockLibraryActions"><button class="libraryExploreBtn" onclick="event.stopPropagation();openPublicList(\''+slug+'\')">Explore &rarr;</button><span class="libraryAdminIdentitySlot">'+ownerActions+'<button class="libraryFollowBtn '+(followed?'following':'')+'" onclick="event.stopPropagation();followPublicList(\''+escapeJsString(list.id)+'\')">'+(followed?'Following':'Follow')+'</button></span></div></div>';
}
function curatedShelfRows(limit=4,query=""){
  const username=currentUsername();
  const libraries=fastCommunityLibraries(Math.max(limit,8),username).map(item=>({type:"library",item}));
  const lists=visiblePublicLists().map(item=>({type:"list",item}));
  const normalizedQuery=String(query||"").toLowerCase().trim();
  const matches=row=>{
    if(!normalizedQuery)return true;
    const item=row.item;
    return `${item.title||""} ${item.username||""} ${item.description||""} ${(item.items||[]).map(album=>`${album.title||""} ${album.artist||""}`).join(" ")}`.toLowerCase().includes(normalizedQuery);
  };
  const availableLibraries=libraries.filter(matches),availableLists=lists.filter(matches);
  const rows=[];
  if(limit<=4){
    if(availableLibraries[0])rows.push(availableLibraries[0]);
    if(availableLists[0])rows.push(availableLists[0]);
    if(availableLists[1])rows.push(availableLists[1]);
    if(availableLibraries[1])rows.push(availableLibraries[1]);
  }else{
    const max=Math.max(availableLibraries.length,availableLists.length);
    for(let index=0;index<max&&rows.length<limit;index++){
      if(availableLibraries[index])rows.push(availableLibraries[index]);
      if(availableLists[index]&&rows.length<limit)rows.push(availableLists[index]);
    }
  }
  const used=new Set(rows.map(row=>`${row.type}:${row.item.id||row.item.slug}`));
  [...availableLibraries,...availableLists].forEach(row=>{
    const key=`${row.type}:${row.item.id||row.item.slug}`;
    if(rows.length<limit&&!used.has(key)){used.add(key);rows.push(row)}
  });
  return rows.slice(0,limit);
}
function libraryBlock(library,context={}){
  const items=liveLibraryItems(Array.isArray(library.items)?library.items:[]);
  const isMine=library.device_id===state.deviceId||library.isMine;
  const isSeeded=isSeededLibrary(library);
  const key=escapeJsString(library.id||library.device_id||"");
  const canFollow=!isSeeded&&!isMine&&library.id&&String(library.id).indexOf("local-library-")!==0;
  const canRemove=!isSeeded&&(isMine||(currentUsername()&&String(library.username||"").toLowerCase()===currentUsername().toLowerCase())||isAdminUnlocked())&&library.id;
  const followers=Number(library.followers_count||0);
  const albumCount=Number(library.album_count||items.length||0);
  ...
  const mineItems=Array.isArray(context.mineItems)?context.mineItems:ownComparableLibraryItems();
  ...
  const similarityRaw=isMine?100:librarySimilarity(items,mineItems);
  ...
  const albumPreview=items.length===1?libraryRowAlbum(items[0]):preview.map((item,index)=>libraryTinyAlbum(item,index===0?"mockFeaturedAlbum":"mockSideAlbum")).join("")+mobileAlbum+(extraCount?'<div class="mockMoreTile">+'+extraCount+'</div>':'');
...
function libraryRecommendationUserKey(){
  return String(loggedInUser()?.id||currentUsername()||state.deviceId||"guest").toLowerCase();
}
function libraryRecommendationCacheKey(){return `muzeLibraryRecommendations:${libraryRecommendationUserKey()}`}
function libraryRecommendationManualKey(){return `${libraryRecommendationCacheKey()}:manual`}
function libraryRecommendationHash(value){
  let hash=2166136261;
...
function clearLibraryRecommendationCache(){localStorage.removeItem(libraryRecommendationCacheKey())}
function noteLibraryRecommendationRating(){
  const key=`muzeLibraryRecommendationRatingCount:${libraryRecommendationUserKey()}`;
  const count=Number(localStorage.getItem(key)||0)+1;
  localStorage.setItem(key,String(count));
  if(count%3===0)clearLibraryRecommendationCache();
}
function selectedLibraryRecommendations(mine,candidates){
  const now=Date.now();
  const signature=mine.map(item=>albumIdentityKey(item)||coverKey(item)||String(item.id||"")).filter(Boolean).sort().join("|");
  const key=libraryRecommendationCacheKey();
  let saved=null;
  try{saved=JSON.parse(localStorage.getItem(key)||"null")}catch(error){saved=null}
  const byId=new Map(candidates.map(album=>[String(album.id||albumIdentityKey(album)||coverKey(album)),album]));
  const savedIds=Array.isArray(saved?.ids)?saved.ids.map(String):[];
  const savedAlbums=savedIds.map(id=>byId.get(id)).filter(Boolean);
  const cacheIsFresh=saved&&saved.signature===signature&&now-Number(saved.refreshedAt||0)<86400000&&savedAlbums.length===Math.min(6,candidates.length);
  if(cacheIsFresh)return savedAlbums;

  const expired=saved&&saved.signature===signature&&savedAlbums.length;
  const retained=expired?savedAlbums.filter((album,index)=>index%2===0).slice(0,3):[];
  const retainedIds=new Set(retained.map(album=>String(album.id||albumIdentityKey(album)||coverKey(album))));
  const previousIds=new Set(expired?savedIds:[]);
  const cycle=Number(saved?.cycle||0)+1;
  const manualSeed=Number(localStorage.getItem(libraryRecommendationManualKey())||0);
  const seed=`${libraryRecommendationUserKey()}|${signature}|${cycle}|${manualSeed}|${Math.floor(now/86400000)}`;
  const freshPool=candidates
    .filter(album=>!retainedIds.has(String(album.id||albumIdentityKey(album)||coverKey(album))))
    .sort((a,b)=>{
      const aId=String(a.id||albumIdentityKey(a)||coverKey(a));
      const bId=String(b.id||albumIdentityKey(b)||coverKey(b));
      const aPrevious=previousIds.has(aId)?1:0;
      const bPrevious=previousIds.has(bId)?1:0;
      return aPrevious-bPrevious||libraryRecommendationHash(`${seed}|${aId}`)-libraryRecommendationHash(`${seed}|${bId}`);
    });
  const fresh=freshPool.slice(0,Math.max(0,6-retained.length));
  const selected=[];
  while((retained.length||fresh.length)&&selected.length<6){
    if(retained.length)selected.push(retained.shift());
    if(fresh.length&&selected.length<6)selected.push(fresh.shift());
  }
  const ids=selected.map(album=>String(album.id||albumIdentityKey(album)||coverKey(album)));
  localStorage.setItem(key,JSON.stringify({signature,refreshedAt:now,cycle,ids}));
  return selected;
}
function libraryRecommendationPanel(){
  const mine=liveLibraryItems(myLibraryItems());
  const mineIds=new Set(mine.flatMap(item=>[String(item.id||""),albumIdentityKey(item),coverKey(item)].filter(Boolean)));
  const candidates=[];
  const seen=new Set();
  for(const album of state.albums){
    const keys=[String(album?.id||""),albumIdentityKey(album),coverKey(album)].filter(Boolean);
    if(keys.some(key=>mineIds.has(key)))continue;
    const identity=keys[0];
    if(!identity||seen.has(identity))continue;
    seen.add(identity);
    candidates.push(album);
  }
  const scoredCandidates=candidates.filter(album=>displayScore(album)!=="-");
  const recommendationPool=(scoredCandidates.length>=6?scoredCandidates:candidates).slice(0,80);
  const recs=selectedLibraryRecommendations(mine,recommendationPool);
  if(!recs.length)return "";
  return '<div class="libraryRecoPanel"><div class="libraryRecoHead"><h3>Inspired by your library</h3><button onclick="findMoreLibraryAlbums()">More recommendations &rarr;</button></div><div>'+recs.map((album,index)=>{
    const score='<em><svg class="libraryInlineIcon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3.2l2.7 5.5 6.1.9-4.4 4.3 1 6.1-5.4-2.9L6.6 20l1-6.1-4.4-4.3 6.1-.9L12 3.2z"></path></svg>'+displayScore(album)+'</em>';
    if(index===0)return '<article class="featuredReco" onclick="openAlbum(\''+escapeJsString(album.id)+'\')">'+cover(album)+'<div class="featuredRecoCopy">'+score+'<strong>'+escapeHtml(album.title)+'</strong><span>'+escapeHtml(album.artist)+'</span><p>Because you love timeless records.</p><div><button onclick="event.stopPropagation();openAlbum(\''+escapeJsString(album.id)+'\')">Open album</button><button class="featuredAddButton" onclick="event.stopPropagation();openAlbum(\''+escapeJsString(album.id)+'\')" aria-label="Open '+escapeHtml(album.title)+'">+</button></div></div></article>';
    return '<article class="standardReco" onclick="openAlbum(\''+escapeJsString(album.id)+'\')">'+cover(album)+'<strong>'+escapeHtml(album.title)+'</strong><span>'+escapeHtml(album.artist)+'</span>'+score+'</article>';
  }).join("")+'</div></div>';
}
function librariesView(){
  const showAllCommunity=Boolean(state.libraryShowAll);
  const displayMine=quickCurrentLibraryCard();
  const query=String(state.librarySearch||"").toLowerCase().trim();
  const communityLimit=showAllCommunity?40:4;
  const curated=curatedShelfRows(communityLimit,query);
  content.innerHTML=`<div class="mockLibrariesHeader"><div><h2>Libraries</h2><p>Explore music through the people who collect it.</p></div><div class="mockLibraryTools"><label><span aria-hidden="true"><svg class="libraryInlineIcon" viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"></circle><path d="m16 16 4 4"></path></svg></span><input value="${escapeHtml(state.librarySearch||"")}" oninput="setLibrarySearch(this.value)" placeholder="Search libraries, suggestions, albums..."></label><button onclick="openPublicListEditor()">+ Add library / suggestion</button></div></div>${ownLibraryHero(displayMine)}<div class="mockCommunityTitle curatedListenersTitle"><div><strong>Curated by Listeners</strong><small>Libraries and suggestions created by music fans like you.</small></div><button onclick="toggleCommunityLibraries()">${showAllCommunity?'Show less':'See all'}</button></div><div class="libraryGrid communityLibraryGrid mockCommunityGrid">${curated.map(row=>row.type==="list"?publicListPreviewBlock(row.item):libraryPreviewBlock(row.item)).join("")||'<div class="empty">No public libraries or suggestions yet.</div>'}</div><div id="libraryRecommendationMount"></div>`;
  setTimeout(()=>{
    const mount=$("#libraryRecommendationMount");
    if(mount&&state.view==="libraries")mount.innerHTML=libraryRecommendationPanel();
  },60);
}
function librariesShellView(){
  const displayMine=quickCurrentLibraryCard();
  content.innerHTML=`<div class="mockLibrariesHeader"><div><h2>Libraries</h2><p>Explore music through the people who collect it.</p></div><div class="mockLibraryTools"><label><span aria-hidden="true"><svg class="libraryInlineIcon" viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"></circle><path d="m16 16 4 4"></path></svg></span><input value="${escapeHtml(state.librarySearch||"")}" oninput="setLibrarySearch(this.value)" placeholder="Search libraries, suggestions, albums..."></label><button onclick="openPublicListEditor()">+ Add library / suggestion</button></div></div>${ownLibraryHero(displayMine)}`;
}
function librariesErrorView(error){
  console.error("Libraries render failed",error);
  const displayMine=quickCurrentLibraryCard();
  content.innerHTML=`<div class="mockLibrariesHeader"><div><h2>Libraries</h2><p>Explore music through the people who collect it.</p></div><div class="mockLibraryTools"><label><span aria-hidden="true"><svg class="libraryInlineIcon" viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"></circle><path d="m16 16 4 4"></path></svg></span><input value="${escapeHtml(state.librarySearch||"")}" oninput="setLibrarySearch(this.value)" placeholder="Search libraries, suggestions, albums..."></label><button onclick="openPublicListEditor()">+ Add library / suggestion</button></div></div>${ownLibraryHero(displayMine)}<div class="mockCommunityTitle curatedListenersTitle"><div><strong>Curated by Listeners</strong><small>Libraries and suggestions created by music fans like you.</small></div></div><div class="libraryGrid communityLibraryGrid mockCommunityGrid"><div class="empty">Curated listener picks could not load. Try refreshing the page.</div></div>`;
}
function publicListView(){
  const list=publicListBySlug(state.publicListSlug||publicListSlugFromPath());
  if(!list){
    content.innerHTML='<section class="publicListPage"><button class="publicListBack" onclick="navigateToView(\'libraries\')">&larr; Libraries</button><div class="empty">This suggestion is unavailable or private.</div></section>';
    return;
  }
  ...
  const editAction=seedIndex>=0?'editSeededLibraryIdentity('+seedIndex+')':'openPublicListEditor(\''+escapeJsString(list.id)+'\')';
  const followed=(extras.listFollows||[]).some(row=>String(row.list_id)===String(list.id)&&String(row.user_id)===String(loggedInUser()?.id||""))||localPublicListFollows().includes(String(list.id));
  const items=liveLibraryItems(list.items||[]);
...
window.openPublicList=function(slug,options={}){
  ...
  document.querySelectorAll(".tab,.navItem[data-view]").forEach(item=>item.classList.toggle("active",item.dataset.view==="libraries"));
  render();window.scrollTo({top:0,behavior:"auto"});
  loadPublicLists().then(()=>{if(state.view==="public-list")render()});
};
...
window.setLibrarySearch=function(value){state.librarySearch=value;state.libraryShowAll=false;librariesView()}
...
window.followPublicList=async function(listId){
  ...
  if(db&&!id.startsWith("seed-list-")){
    const result=alreadyFollowed
      ?await db.from("list_follows").delete().eq("list_id",id).eq("user_id",loggedInUser().id)
      :await db.from("list_follows").insert({list_id:id,user_id:loggedInUser().id});
    if(result.error&&!publicListSchemaUnavailable(result.error)){alert(result.error.message);return}
  }
  const local=localPublicListFollows().filter(value=>value!==id);
  if(!alreadyFollowed)local.push(id);
  saveLocalPublicListFollows(local);
...
function listEditorCandidates(){
  const rows=dedupeAlbumRows([...(state.listDraft?.originalItems||[]),...myLibraryItems(),...(state.albums||[])]);
  return rows.slice(0,160).map(album=>album.id?album:albumToLibraryItem(album));
}
function ensurePublicListModal(){
  let modal=$("#publicListModal");
  if(modal)return modal;
  modal=document.createElement("div");
  modal.id="publicListModal";modal.className="modal publicListModal hidden";
  modal.innerHTML='<div class="modalPanel publicListEditorPanel" role="dialog" aria-modal="true" aria-labelledby="publicListEditorTitle"><button class="close cleanClose" aria-label="Close suggestion editor" onclick="closePublicListEditor()"><span class="cleanCloseIcon" aria-hidden="true"></span></button><div id="publicListEditorContent"></div></div>';
  modal.addEventListener("click",event=>{if(event.target===modal)closePublicListEditor()});
  document.body.append(modal);return modal;
}
function listDraftAlbum(id){return listEditorCandidates().find(album=>String(album.id)===String(id))||null}
function renderPublicListEditor(){
  const draft=state.listDraft;if(!draft)return;
  const mount=$("#publicListEditorContent");if(!mount)return;
  const query=String(draft.search||"").toLowerCase();
  const candidates=listEditorCandidates().filter(album=>!query||`${album.title||""} ${album.artist||""}`.toLowerCase().includes(query)).slice(0,60);
  const selected=draft.albumIds.map(listDraftAlbum).filter(Boolean);
  const usernameField=draft.canEditUsername?'<label>Username<input id="publicListUsername" maxlength="32" value="'+escapeHtml(draft.username||"")+'" placeholder="listener"></label>':'';
  const typeField=draft.canEditType?'<label>Content type<select id="publicListCurationType"><option value="list" '+(draft.curation_type==="list"?'selected':'')+'>Suggestion</option><option value="library" '+(draft.curation_type==="library"?'selected':'')+'>Library</option></select></label>':'';
  mount.innerHTML='<p class="eyebrow">'+(draft.canEditUsername?'Admin curation':'Listener curation')+'</p><h2 id="publicListEditorTitle">'+(draft.id?'Edit curation':'Create a public suggestion')+'</h2><div class="publicListForm"><label>Title<input id="publicListTitle" maxlength="70" value="'+escapeHtml(draft.title)+'" placeholder="Summer Breezers"></label>'+usernameField+typeField+'<label>Description<textarea id="publicListDescription" maxlength="220" rows="3" placeholder="What connects these records?">'+escapeHtml(draft.description)+'</textarea></label><label class="publicListVisibility"><input id="publicListVisibility" type="checkbox" '+(draft.is_public?'checked':'')+'> Make this curation public</label><label>Find albums<input value="'+escapeHtml(draft.search||"")+'" oninput="setPublicListAlbumSearch(this.value)" placeholder="Search your library and Muze..."></label><div class="publicListAlbumPicker">'+candidates.map(album=>'<button type="button" class="publicListAlbumChoice '+(draft.albumIds.includes(String(album.id))?'selected':'')+'" onclick="togglePublicListDraftAlbum(\''+escapeJsString(album.id)+'\')">'+(album.cover_url?'<img src="'+escapeHtml(album.cover_url)+'" alt="">':'<span></span>')+'<b>'+escapeHtml(album.title||"Untitled")+'</b><small>'+escapeHtml(album.artist||"")+'</small></button>').join("")+'</div><div class="publicListOrder"><strong>Album order</strong>'+(selected.length?selected.map((album,index)=>'<div><span>'+(index+1)+'. '+escapeHtml(album.title||"Untitled")+'</span><button type="button" onclick="movePublicListDraftAlbum('+index+',-1)" aria-label="Move '+escapeHtml(album.title||"album")+' up">&uarr;</button><button type="button" onclick="movePublicListDraftAlbum('+index+',1)" aria-label="Move '+escapeHtml(album.title||"album")+' down">&darr;</button><button type="button" onclick="togglePublicListDraftAlbum(\''+escapeJsString(album.id)+'\')" aria-label="Remove '+escapeHtml(album.title||"album")+'">&times;</button></div>').join(""):'<p>Select at least one album.</p>')+'</div><div class="publicListFormActions">'+(draft.id&&draft.canDelete?'<button type="button" class="publicListDelete" onclick="deletePublicListDraft()">Delete suggestion</button>':'<span></span>')+'<button type="button" onclick="savePublicListDraft()">Save changes</button></div></div>';
}
window.openPublicListEditor=function(listId=""){
  const existing=listId?visiblePublicLists().find(list=>String(list.id)===String(listId)):null;
  const owned=publicListOwnedByUser(existing);
  const adminEditing=Boolean(existing)&&!owned&&isAdminUnlocked();
  if(!adminEditing&&!requireAuth("save",()=>openPublicListEditor(listId)))return;
  if(existing&&!owned&&!adminEditing){alert("Only the suggestion creator or a Muze admin can edit this suggestion.");return}
  state.listDraft={id:existing?.id||"",title:existing?.title||"",username:existing?.username||currentUsername()||"listener",description:existing?.description||"",curation_type:publicListCurationType(existing||{}),is_public:existing?.is_public!==false,slug:existing?.slug||"",albumIds:(existing?.items||[]).map(album=>String(album.id)),originalItems:existing?.items||[],adminEditing,canEditUsername:Boolean(existing)&&isAdminUnlocked(),canEditType:Boolean(existing)&&isAdminUnlocked(),canDelete:Boolean(existing)&&owned,search:""};
  ensurePublicListModal().classList.remove("hidden");renderPublicListEditor();
};
window.closePublicListEditor=function(){state.listDraft=null;$("#publicListModal")?.classList.add("hidden")};
window.setPublicListAlbumSearch=function(value){if(!state.listDraft)return;state.listDraft.search=value;renderPublicListEditor()};
window.togglePublicListDraftAlbum=function(id){
  if(!state.listDraft)return;id=String(id);
  state.listDraft.albumIds=state.listDraft.albumIds.includes(id)?state.listDraft.albumIds.filter(value=>value!==id):[...state.listDraft.albumIds,id];
  renderPublicListEditor();
};
window.movePublicListDraftAlbum=function(index,direction){
  if(!state.listDraft)return;
  const next=index+direction;if(next<0||next>=state.listDraft.albumIds.length)return;
  const rows=state.listDraft.albumIds.slice();[rows[index],rows[next]]=[rows[next],rows[index]];state.listDraft.albumIds=rows;renderPublicListEditor();
};
window.savePublicListDraft=async function(){
  const draft=state.listDraft;if(!draft)return;
  const title=String($("#publicListTitle")?.value||"").trim();
  const username=draft.canEditUsername?String($("#publicListUsername")?.value||"").trim().replace(/^@+/,""):String(draft.username||"").trim().replace(/^@+/,"");
  const curationType=draft.canEditType?publicListCurationType({curation_type:$("#publicListCurationType")?.value}):"list";
  const description=String($("#publicListDescription")?.value||"").trim();
  const isPublic=Boolean($("#publicListVisibility")?.checked);
  if(!title){alert("Give this suggestion a title.");return}
  if(draft.canEditUsername&&!/^[a-zA-Z0-9._-]{2,32}$/.test(username)){alert("Use 2-32 letters, numbers, dots, underscores, or hyphens for the username.");return}
  if(!draft.albumIds.length){alert("Choose at least one album.");return}
  const user=loggedInUser();
  const existing=draft.id?visiblePublicLists().find(list=>String(list.id)===String(draft.id)):null;
  const baseSlug=publicListSlug(draft.slug||title);
  const collision=visiblePublicLists().some(list=>String(list.id)!==String(draft.id)&&publicListSlug(list.slug||list.title)===baseSlug);
  const slug=collision?`${publicListSlug(title)}-${Date.now().toString(36).slice(-5)}`:baseSlug;
  const items=draft.albumIds.map(listDraftAlbum).filter(Boolean).map(albumToLibraryItem);
  const now=new Date().toISOString();
  const editingExisting=Boolean(existing);
  let saved={...existing,id:draft.id||`local-list-${crypto.randomUUID()}`,user_id:editingExisting?(existing?.user_id||null):(user?.id||null),local_owner_device_id:editingExisting?(existing?.local_owner_device_id||""):state.deviceId,username:draft.canEditUsername?username:(editingExisting?(existing?.username||"listener"):(currentUsername()||"listener")),title,slug,description,curation_type:curationType,is_public:isPublic,items,album_count:items.length,followers_count:Number(existing?.followers_count||0),created_at:existing?.created_at||now,updated_at:now};
  if(db&&user&&!String(draft.id).startsWith("seed-list-")){
    const payload={title,slug,description,curation_type:curationType,is_public:isPublic,updated_at:now};
    let result=draft.id&&!String(draft.id).startsWith("local-list-")
      ?await db.from("lists").update(payload).eq("id",draft.id).select().single()
      :await db.from("lists").insert({...payload,user_id:user.id,created_at:now}).select().single();
    if(result.error&&/curation_type|column/i.test(String(result.error.message||""))){
      const legacyPayload={title,slug,description,is_public:isPublic,updated_at:now};
      result=draft.id&&!String(draft.id).startsWith("local-list-")
        ?await db.from("lists").update(legacyPayload).eq("id",draft.id).select().single()
        :await db.from("lists").insert({...legacyPayload,user_id:user.id,created_at:now}).select().single();
    }
    if(!result.error){
      saved={...saved,...result.data,id:result.data.id};
      const validAlbums=draft.albumIds.filter(id=>/^[0-9a-f]{8}-[0-9a-f-]{27}$/i.test(id));
      const removeResult=await db.from("list_albums").delete().eq("list_id",saved.id);
      if(!removeResult.error&&validAlbums.length){
        const insertResult=await db.from("list_albums").insert(validAlbums.map((album_id,position)=>({list_id:saved.id,album_id,position})));
        if(insertResult.error)result={error:insertResult.error};
      }
    }
    if(result.error&&!publicListSchemaUnavailable(result.error)){alert(result.error.message);return}
  }
  const local=localPublicLists().filter(list=>String(list.id)!==String(draft.id)&&String(list.slug)!==String(saved.slug));
  if(draft.canEditType)savePublicListTypeOverride(saved,curationType);
  saveLocalPublicLists([saved,...local]);
  extras.publicLists=[saved,...(extras.publicLists||[]).filter(list=>String(list.id)!==String(draft.id)&&String(list.id)!==String(saved.id))];
  closePublicListEditor();
  if(state.view==="libraries")librariesView();else openPublicList(saved.slug,{replace:true});
};
window.deletePublicListDraft=async function(){
  const draft=state.listDraft;if(!draft)return;
  if(!draft.canDelete){alert("Only the suggestion creator can delete this suggestion.");return}
  if(!confirm("Delete this suggestion?"))return;
  if(db&&loggedInUser()&&!String(draft.id).startsWith("local-list-")){
    const {error}=await db.from("lists").delete().eq("id",draft.id).eq("user_id",loggedInUser().id);
    if(error&&!publicListSchemaUnavailable(error)){alert(error.message);return}
  }
  saveLocalPublicLists(localPublicLists().filter(list=>String(list.id)!==String(draft.id)));
  extras.publicLists=(extras.publicLists||[]).filter(list=>String(list.id)!==String(draft.id));
  closePublicListEditor();navigateToView("libraries");
};
...
window.findMoreLibraryAlbums=function(){
  const manualKey=libraryRecommendationManualKey();
  localStorage.setItem(manualKey,String(Number(localStorage.getItem(manualKey)||0)+1));
  clearLibraryRecommendationCache();
  const mount=$("#libraryRecommendationMount");
  if(mount&&state.view==="libraries")mount.innerHTML=libraryRecommendationPanel();
}
window.openAlbumOverviewFilter=function(type,value){
  ...
  requestHomepageAlbumPage({reset:true,force:true});
...
function genres(){return["All",...new Set([...(state.homeGenres||[]),...state.albums.map(albumGenreLabel).filter(Boolean),"Greatest hits","Classical","Soundtracks","International"])]}
function filtered(){
  const rows=state.albums.slice();
  const explicitlyRequested=Boolean(String(state.search||"").trim())||genreKey(state.genre)==="greatest hits";
  return explicitlyRequested?rows:rows.filter(album=>!albumIsGreatestHits(album));
}
function muzeSearchSuggestions(query){
  const q=fuzzySearchKey(query);
  if(q.length<1)return [];
  return state.albums.map(album=>{
    const title=fuzzySearchKey(album.title);
    const artist=fuzzySearchKey(album.artist);
    const genre=fuzzySearchKey(cachedAlbumGenreLabel(album));
    const haystack=albumSearchText(album);
    if(!albumMatchesSearch(album,q))return null;
    let rank=0;
    if(title===q)rank+=1000;
    else if(title.startsWith(q))rank+=650;
    else if(title.includes(q))rank+=360;
    if(artist===q)rank+=760;
    else if(artist.startsWith(q))rank+=460;
    else if(artist.includes(q))rank+=230;
    if(genre.startsWith(q))rank+=120;
    if(!haystack.includes(q))rank+=95;
    rank+=Math.min(100,score(album)*10)+Math.min(80,count(album)/500);
    return {album,rank};
  }).filter(Boolean).sort((a,b)=>b.rank-a.rank||String(a.album.title||"").localeCompare(String(b.album.title||""))).slice(0,7).map(item=>item.album);
}
function muzeArtistSearchSuggestions(query){
  const q=fuzzySearchKey(query);
  if(q.length<2)return [];
  return artistDirectoryBaseRows().map(row=>{
    const name=fuzzySearchKey(row.name);
    const haystack=fuzzySearchKey(row.searchText);
    const tokens=name.split(" ").filter(Boolean);
    const qTokens=q.split(" ").filter(Boolean);
    const tokenMatch=qTokens.every(token=>tokens.some(candidate=>candidate.includes(token)||token.includes(candidate)||editDistanceWithin(token,candidate,token.length<=5?1:2)));
    if(!(name===q||name.startsWith(q)||name.includes(q)||haystack.includes(q)||tokenMatch))return null;
    let rank=0;
    if(name===q)rank+=1600;
    else if(name.startsWith(q))rank+=1100;
    else if(name.includes(q))rank+=720;
    else if(tokenMatch)rank+=520;
    if(haystack.includes(q))rank+=140;
    rank+=Math.min(140,Number(row.totalRatings||0)/250)+Math.min(80,Number(row.rank?.avg||0)*8);
    return {row,rank};
  }).filter(Boolean).sort((a,b)=>b.rank-a.rank||String(a.row.name||"").localeCompare(String(b.row.name||""))).slice(0,2).map(item=>item.row);
}
function muzeSearchSuggestionItems(query){
  const artists=muzeArtistSearchSuggestions(query);
  const artistItems=artists.map(row=>({type:"artist",row}));
...
function muzeSearchAlbumRuntime(album){
  const direct=Number(album?.total_runtime_ms||album?.runtime_ms||album?.duration_ms||0);
  if(direct>0)return albumInfoRuntime(direct);
  const info=extras.albumInfo?.[albumRef(album?.id)];
  const metadata=info?.metadata||{};
  const saved=Number(metadata.total_runtime_ms||info?.total_runtime_ms||0);
  if(saved>0)return albumInfoRuntime(saved);
  const tracks=extras.tracks?.[albumRef(album?.id)];
  if(Array.isArray(tracks)&&tracks.length){
    const total=tracks.reduce((sum,track)=>sum+Math.max(0,Number(track.duration_ms)||0),0);
    if(total>0)return albumInfoRuntime(total);
  }
  return "";
}
function renderMuzeSearchSuggestions(){
  const box=$("#muzeSearchSuggest");
  const input=$("#searchInput");
  if(!box||!input)return;
  const rows=muzeSearchSuggestionItems(input.value);
  if(!rows.length){hideMuzeSearchSuggestions();return}
  box.innerHTML=rows.map(item=>{
    ...
    const meta=[album.year,cachedAlbumGenreLabel(album)].filter(Boolean).join("  ");
    const runtime=muzeSearchAlbumRuntime(album);
...
function ensureMuzeSearchDropdownVisible(){
  ...
  requestAnimationFrame(()=>{
    ...
    const bottomOverflow=rect.bottom-viewportBottom+12;
    const topOverflow=viewportTop-rect.top+12;
    if(bottomOverflow>0)window.scrollBy({top:bottomOverflow,behavior:"auto"});
    else if(topOverflow>0)window.scrollBy({top:-topOverflow,behavior:"auto"});
  });
}
function scheduleMuzeSearchSuggestions(){
  clearTimeout(muzeSearchSuggestTimer);
  muzeSearchSuggestTimer=setTimeout(()=>{
    muzeSearchSuggestTimer=0;
...
function scheduleHomepageCatalogueSearch(query,after){
  ...
  homepageCatalogueSearchTimer=setTimeout(async()=>{
    try{
      ...
      clearArtistDirectoryCache();
      after?.();
    }catch(error){console.warn("[Muze search] catalogue search failed",error?.message||error)}
  },180);
}
window.selectMuzeSearchSuggestion=function(albumId){
  hideMuzeSearchSuggestions();
  openAlbum(albumId);
};
window.selectMuzeArtistSuggestion=function(slug){
  hideMuzeSearchSuggestions();
  openArtistPage(slug);
};
function scheduleHomeSearchRender(){
  clearTimeout(homeSearchRenderTimer);
  homeSearchRenderTimer=setTimeout(async()=>{homeSearchRenderTimer=0;await requestHomepageAlbumPage({reset:true,force:true});renderMuzeSearchSuggestions()},isMobileViewport()?320:180);
}
function card(a){const genreLabel=cachedAlbumGenreLabel(a);const adminDelete=isAdminUnlocked()?`<button class="adminAlbumDeleteBtn" onclick="event.stopPropagation();deleteAlbumAdmin('${escapeJsString(a.id)}')" aria-label="Delete ${escapeHtml(a.title)}">Delete</button>`:"";return`<article class="card albumCard" onclick="openAlbum('${escapeJsString(a.id)}')">${cover(a)}<button class="quickLibraryAdd" onclick="event.stopPropagation();addCurrentAlbumToLibrary('${escapeJsString(a.id)}')">+ Add to my library</button>${adminDelete}<div class="cardBody"><div class="row"><div><div class="title">${escapeHtml(a.title)}</div><div class="artist">${escapeHtml(a.artist)} - ${escapeHtml(a.year||"")}</div></div><div class="score">${displayScore(a)}</div></div><span class="pill">${escapeHtml(genreLabel)}</span></div></article>`}
function desktopAlbumListIsActive(){return isAdminUnlocked()&&state.albumCatalogueView==="list"&&!isMobileViewport()}
function albumCatalogueViewControls(){
  if(!isAdminUnlocked())return "";
  const selected=desktopAlbumListIsActive()?"list":"grid";
  return `<div class="albumCatalogueViewSelector" role="group" aria-label="Album view">
    <button type="button" class="${selected==="grid"?"selected":""}" aria-label="View albums as grid" aria-pressed="${selected==="grid"}" title="View albums as grid" onclick="setAlbumCatalogueView('grid')"><svg viewBox="0 0 16 16" aria-hidden="true" focusable="false"><rect x="1.5" y="1.5" width="5" height="5" rx="1"></rect><rect x="9.5" y="1.5" width="5" height="5" rx="1"></rect><rect x="1.5" y="9.5" width="5" height="5" rx="1"></rect><rect x="9.5" y="9.5" width="5" height="5" rx="1"></rect></svg><span>Grid</span></button>
    <button type="button" class="${selected==="list"?"selected":""}" aria-label="View albums as list" aria-pressed="${selected==="list"}" title="View albums as list" onclick="setAlbumCatalogueView('list')"><svg viewBox="0 0 16 16" aria-hidden="true" focusable="false"><path d="M2 3.5h2M6 3.5h8M2 8h2M6 8h8M2 12.5h2M6 12.5h8"></path></svg><span>List</span></button>
  </div>`;
}
window.setAlbumCatalogueView=function(view){
  if(!isAdminUnlocked()||isMobileViewport()||!["grid","list"].includes(view)||state.albumCatalogueView===view)return;
  const scrollPosition=window.scrollY;
  state.albumCatalogueView=view;
  try{localStorage.setItem(ALBUM_CATALOGUE_VIEW_KEY,view)}catch(error){}
  renderRankingsView();
  const restoreScroll=()=>window.scrollTo({top:scrollPosition,behavior:"auto"});
  requestAnimationFrame(()=>{restoreScroll();requestAnimationFrame(restoreScroll);setTimeout(restoreScroll,60)});
}
const albumCatalogueDesktopBreakpoint=window.matchMedia("(max-width:850px)");
albumCatalogueDesktopBreakpoint.addEventListener?.("change",()=>{
  if(state.view==="rankings"&&state.dataReady)renderRankingsView();
});
let albumCatalogueWasMobile=isMobileViewport();
window.addEventListener("resize",()=>{
  const mobile=isMobileViewport();
  if(mobile===albumCatalogueWasMobile)return;
  albumCatalogueWasMobile=mobile;
  if(state.view==="rankings"&&state.dataReady)renderRankingsView();
});
function albumCatalogueListArtwork(a){
  const url=albumCoverUrl(a);
  const image=url?`<img src="${escapeHtml(url)}" loading="lazy" onerror="this.hidden=true" alt="${escapeHtml(a.title||"Album cover")}">`:"";
  return `<div class="albumCatalogueListArtwork"><div class="cover">${image}${fallback(a)}</div><button type="button" class="albumCatalogueListAdd" onclick="event.stopPropagation();addCurrentAlbumToLibrary('${escapeJsString(a.id)}')" aria-label="Add ${escapeHtml(a.title)} to my library" title="Add to my library">+</button></div>`;
}
function albumCatalogueListRow(a){
  const rank=muzeTop250Rank(a);
  const genreLabel=cachedAlbumGenreLabel(a);
  const genre=genreLabel&&genreKey(genreLabel)!=="album"?`<span class="albumCatalogueListGenre">${escapeHtml(genreLabel)}</span>`:"";
  const metadata=[a.artist,a.year].filter(value=>String(value||"").trim()).map(escapeHtml).join(" &middot; ");
  const adminDelete=isAdminUnlocked()?`<button class="adminAlbumDeleteBtn" onclick="event.stopPropagation();deleteAlbumAdmin('${escapeJsString(a.id)}')" aria-label="Delete ${escapeHtml(a.title)}">Delete</button>`:"";
  return `<article class="albumCatalogueListRow" role="button" tabindex="0" aria-label="Open ${escapeHtml(a.title)} album details" onclick="openAlbum('${escapeJsString(a.id)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openAlbum('${escapeJsString(a.id)}')}">
    <div class="albumCatalogueListRankCell">${rank?`<span class="score albumCatalogueListRankValue">#${rank}</span>`:""}</div>
    ${albumCatalogueListArtwork(a)}
    <div class="albumCatalogueListCopy"><h3>${escapeHtml(a.title)}</h3>${metadata?`<p>${metadata}</p>`:""}${genre}</div>
    <div class="score albumCatalogueListScore" aria-label="Muze score ${displayScore(a)}">${displayScore(a)}</div>${adminDelete}
  </article>`;
}
...
function renderRankingsView(){
  ++albumRenderToken;
  const arr=filtered();
  let top=state.homeTopAlbum||state.albums.slice().sort((a,b)=>score(b)-score(a))[0];
  if(top){
    $("#heroScore").textContent=displayScore(top);$("#heroTitle").textContent=top.title;
    const heroCard=$("#heroCard");
    if(heroCard){heroCard.setAttribute("onclick",`openAlbum('${escapeJsString(top.id)}')`);heroCard.setAttribute("role","button");heroCard.setAttribute("tabindex","0");heroCard.setAttribute("aria-label",`Open ${top.title||"top-rated album"} album details`);heroCard.onkeydown=event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();openAlbum(top.id)}};if(top.cover_url){heroCard.style.setProperty("--hero-cover",`url("${top.cover_url}")`)}else{heroCard.style.removeProperty("--hero-cover")}}
  }
  $("#genreFilter").innerHTML=genres().map(g=>`<option value="${escapeHtml(g)}" ${g===state.genre?"selected":""}>${escapeHtml(g==="All"?"All genres":g)}</option>`).join("");
  if(state.homeLoading&&!arr.length){content.innerHTML=homepageSkeletonMarkup();return}
  const visible=[...arr.filter(album=>album._justAddedSpotify),...arr.filter(album=>!album._justAddedSpotify)];
  const more=state.homeHasMore?`<div class="albumBatchMore"><button onclick="showMoreAlbums()" ${state.homeLoading?"disabled aria-busy=\"true\"":""}>${state.homeLoading?"Loading…":"See more"}</button></div>`:"";
  const resultCount=isAdminUnlocked()&&Number.isFinite(state.homeTotal)?`<span class="muted">${state.homeTotal} albums</span>`:"";
  const heading=state.sort==="hidden"?"Hidden Gems":"Top Albums";
  const results=desktopAlbumListIsActive()
    ?`<div class="album-results album-results--list">${visible.map(albumCatalogueListRow).join("")}</div>`
    :`<div class="album-results album-results--grid grid">${visible.map(card).join("")}</div>`;
  content.innerHTML=`<div class="sectionTitle albumCatalogueSectionTitle"><h2>${heading}</h2><div class="albumCatalogueHeaderActions">${resultCount}${albumCatalogueViewControls()}</div></div>${results}${more}`;
}
function homepageSkeletonMarkup(){
  const status=state.homeLoadMessage?`<p class="homepageLoadStatus">${escapeHtml(state.homeLoadMessage)}</p>`:"";
  const failure=state.homeLoadState==="error"?`<div class="homepageLoadFailure"><span>We couldn’t load the rankings.</span><button type="button" onclick="retryHomepageRankings()">Retry</button></div>`:"";
  const skeleton=desktopAlbumListIsActive()
    ?`<div class="album-results album-results--list homepageListSkeleton" aria-hidden="true">${Array.from({length:6},()=>'<article class="homepageListRowSkeleton"></article>').join("")}</div>`
    :`<div class="album-results album-results--grid grid homepageSkeletonGrid" aria-hidden="true">${Array.from({length:4},()=>'<article class="card albumCard homepageCardSkeleton"></article>').join("")}</div>`;
  return `<div class="sectionTitle albumCatalogueSectionTitle"><h2>Top Albums</h2><div class="albumCatalogueHeaderActions">${albumCatalogueViewControls()}</div></div>${skeleton}${status}${failure}`;
}
function render(){
if(state.view!=="artists")stopTrendingArtistFlip();
if(!state.dataReady){const heroScore=$("#heroScore"),heroTitle=$("#heroTitle");if(heroScore){heroScore.textContent="";heroScore.classList.add("homepageSkeletonValue");heroScore.setAttribute("aria-hidden","true")}if(heroTitle){heroTitle.textContent="";heroTitle.classList.add("homepageSkeletonTitle");heroTitle.setAttribute("aria-hidden","true")}const heroCard=$("#heroCard");if(heroCard){heroCard.style.removeProperty("--hero-cover");heroCard.removeAttribute("onclick");heroCard.removeAttribute("role");heroCard.removeAttribute("tabindex");heroCard.removeAttribute("aria-label");heroCard.onkeydown=null}content.innerHTML=homepageSkeletonMarkup();return}
$("#heroScore")?.classList.remove("homepageSkeletonValue");$("#heroScore")?.removeAttribute("aria-hidden");$("#heroTitle")?.classList.remove("homepageSkeletonTitle");$("#heroTitle")?.removeAttribute("aria-hidden");
...
document.body.classList.toggle("artistView",state.view==="artists");
document.body.classList.toggle("artistProfileView",state.view==="artist-profile");
document.body.classList.toggle("libraryView",state.view==="libraries"||state.view==="public-list");
...
window.showMoreAlbums=function(){if(state.homeLoading||!state.homeHasMore)return;return requestHomepageAlbumPage({reset:false})}
...
const commaArtistNameExceptions=new Set(["earth, wind & fire","earth, wind and fire","crosby, stills, nash & young","crosby, stills, nash and young","emerson, lake & palmer","emerson, lake and palmer","blood, sweat & tears","blood, sweat and tears"]);
function artistCreditNames(value){
  const raw=String(value||"").trim();
  if(!raw)return [];
  if(commaArtistNameExceptions.has(raw.toLowerCase()))return [raw];
  const parts=raw.split(/\s*,\s*/).map(part=>part.trim()).filter(Boolean);
  return parts.length>1?parts:[raw];
}
function albumHasArtistCredit(album,slug){
  return artistCreditNames(album?.artist).some(name=>artistSlugForName(name)===slug);
}
function artistDirectoryBaseRows(){
  ...
  state.albums.forEach(album=>{
    artistCreditNames(album.artist).forEach(name=>{
...
function artistTrendNameKey(value){return String(value||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/^the\s+/,"").replace(/&/g,"and").replace(/[^a-z0-9]+/g," ").trim()}
function trendingArtistDayIndex(){
  const now=new Date();
  const start=new Date(now.getFullYear(),now.getMonth(),now.getDate());
  return Math.floor(start.getTime()/86400000);
}
function dailyTrendingArtistSlice(rows){
  if(!rows.length)return [];
  const start=(trendingArtistDayIndex()*4)%rows.length;
  const ordered=[...rows.slice(start),...rows.slice(0,start)];
  const selected=[];
  for(const row of ordered){
    if(uniqueTrendingArtistCoverSets([...selected,row]).every(covers=>covers.length))selected.push(row);
    if(selected.length===4)break;
  }
  return selected;
}
function matchedTrendingArtists(){
  const rows=artistDirectoryBaseRows();
  const byName=new Map(rows.map(row=>[artistTrendNameKey(row.name),row]));
  const used=new Set();
  const matched=[];
  (extras.trendingArtists||[]).forEach(trend=>{
    const row=byName.get(artistTrendNameKey(trend.name));
    if(!row||used.has(row.slug))return;
    used.add(row.slug);
    matched.push({...row,trend});
  });
  rows.forEach(row=>{
    if(used.has(row.slug))return;
    used.add(row.slug);
    matched.push({...row,trend:{rank:matched.length+1,sources:["Muze community"],fallback:true}});
  });
  return dailyTrendingArtistSlice(matched);
}
let trendingArtistFlipTimer=0;
let trendingArtistFlipOrderIndex=0;
let trendingArtistDailyTimer=0;
function stopTrendingArtistFlip(){
  clearInterval(trendingArtistFlipTimer);
  trendingArtistFlipTimer=0;
  clearTimeout(trendingArtistDailyTimer);
  trendingArtistDailyTimer=0;
}
function trendingArtistCovers(row){
  const hero=row.hero||row.featured?.[0]||{};
  const covers=[hero,...(row.featured||[]),...(row.albums||[])].map(album=>({url:String(album?.cover_url||"").trim(),title:String(album?.title||row.name||"Artist image").trim()})).filter(item=>item.url);
  const seen=new Set();
  return covers.filter(item=>{const key=trendingArtistCoverKey(item.url);if(seen.has(key))return false;seen.add(key);return true}).slice(0,12);
}
function trendingArtistCoverKey(value){return String(value||"").trim().toLowerCase()}
function uniqueTrendingArtistCoverSets(rows){
  const allCovers=rows.map(trendingArtistCovers);
  const coverOwner=new Map();
  const selected=new Array(rows.length).fill(-1);
  function assign(rowIndex,visited){
    for(let coverIndex=0;coverIndex<allCovers[rowIndex].length;coverIndex++){
      const key=trendingArtistCoverKey(allCovers[rowIndex][coverIndex].url);
      if(visited.has(key))continue;
      visited.add(key);
      const previousOwner=coverOwner.get(key);
      if(previousOwner===undefined||assign(previousOwner,visited)){
        coverOwner.set(key,rowIndex);
        selected[rowIndex]=coverIndex;
        return true;
      }
    }
    return false;
  }
  rows.forEach((row,rowIndex)=>assign(rowIndex,new Set()));
  return allCovers.map((covers,rowIndex)=>{
    const selectedIndex=selected[rowIndex];
    return selectedIndex<0?[]:[covers[selectedIndex],...covers.slice(selectedIndex+1),...covers.slice(0,selectedIndex)];
  });
}
function startTrendingArtistFlip(){
  clearInterval(trendingArtistFlipTimer);
  const cards=[...document.querySelectorAll(".trendingArtistCard[data-cover-count]")].filter(card=>Number(card.dataset.coverCount)>1);
  if(!cards.length)return;
  const orders=[[0,2,1,3],[0,3,2,1],[1,3,0,2],[2,0,3,1],[1,0,3,2],[2,3,0,1]];
  const flip=()=>{
    const order=orders[trendingArtistFlipOrderIndex%orders.length];
    trendingArtistFlipOrderIndex++;
    order.map(index=>cards[index]).filter(Boolean).forEach((card,index)=>setTimeout(()=>{
    if(!document.body.contains(card))return;
    const images=[...card.querySelectorAll(".trendingArtistArt img")];
    if(images.length<2)return;
    const current=Number(card.dataset.coverIndex||0);
    const used=new Set([...document.querySelectorAll(".trendingArtistCard .trendingArtistArt img.active")]
      .filter(image=>!card.contains(image))
      .map(image=>trendingArtistCoverKey(image.dataset.coverKey||image.currentSrc||image.src)));
    let next=current;
    for(let offset=1;offset<images.length;offset++){
      const candidate=(current+offset)%images.length;
      if(!used.has(trendingArtistCoverKey(images[candidate].dataset.coverKey||images[candidate].currentSrc||images[candidate].src))){next=candidate;break}
    }
    if(next===current)return;
    card.dataset.coverIndex=String(next);
    images.forEach((image,imageIndex)=>image.classList.toggle("active",imageIndex===next));
  },index*320));
  };
  trendingArtistFlipTimer=setInterval(flip,3000);
}
function trendingArtistCard(row,index,covers=trendingArtistCovers(row)){
  const genres=(row.genres||[]).slice(0,2).join(" / ");
  const source=row.trend?.sources?.[0]||"Listener activity";
  const label=row.trend?.fallback?"Muze pick":`Trending #${row.trend?.rank||index+1}`;
  const art=covers.length?covers.map((cover,coverIndex)=>`<img class="${coverIndex===0?"active":""}" data-cover-key="${escapeHtml(trendingArtistCoverKey(cover.url))}" src="${escapeHtml(cover.url)}" loading="${coverIndex?"lazy":"eager"}" decoding="async" onerror="this.hidden=true" alt="${escapeHtml(row.name)} - ${escapeHtml(cover.title)}">`).join(""):`<span>${escapeHtml(coverText({title:row.name}))}</span>`;
  return `<article class="trendingArtistCard" data-cover-count="${covers.length}" data-cover-index="0" role="button" tabindex="0" onclick="openArtistPage('${escapeJsString(row.slug)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openArtistPage('${escapeJsString(row.slug)}')}"><div class="trendingArtistArt">${art}</div><div class="trendingArtistCopy"><span>${escapeHtml(label)}</span><h3>${escapeHtml(row.name)}</h3><p>${escapeHtml(genres||source)}</p><small>${escapeHtml(source)}</small></div></article>`;
}
function trendingArtistsMarkup(){
  const rows=matchedTrendingArtists();
  if(!rows.length)return "";
  const coverSets=uniqueTrendingArtistCoverSets(rows);
  return `<section class="trendingArtistsBlock" aria-label="Trending artists"><div class="trendingArtistsHeader"><div><p class="artistFilterLabel">Trending artists</p><h3>Artists people are playing now</h3></div></div><div class="trendingArtistsGrid">${rows.map((row,index)=>trendingArtistCard(row,index,coverSets[index])).join("")}</div></section>`;
}
function renderTrendingArtists(){
  const host=$("#artistTrending");
  if(host)host.outerHTML=`<div id="artistTrending">${trendingArtistsMarkup()}</div>`;
  startTrendingArtistFlip();
  scheduleTrendingArtistsDailyRefresh();
}
function scheduleTrendingArtistsDailyRefresh(){
  clearTimeout(trendingArtistDailyTimer);
  const now=new Date();
  const next=new Date(now.getFullYear(),now.getMonth(),now.getDate()+1,0,0,2);
  trendingArtistDailyTimer=setTimeout(()=>{
    if(state.view==="artists")renderTrendingArtists();
  },Math.max(1000,next.getTime()-now.getTime()));
}
async function loadTrendingArtists(){
  if(extras.trendingArtistsRequest)return extras.trendingArtistsRequest;
  const cached=(()=>{try{return JSON.parse(localStorage.getItem("muzeTrendingArtists:v1")||"null")}catch(error){return null}})();
  if(cached?.artists&&Date.now()-Number(cached.saved_at||0)<6*60*60*1000){extras.trendingArtists=cached.artists;extras.trendingArtistsLoaded=true;renderTrendingArtists()}
  extras.trendingArtistsRequest=fetchWithTimeout("/.netlify/functions/trending-artists",{cache:"no-store"},9000)
    .then(response=>response.ok?response.json():Promise.reject(new Error("Trending artists request failed")))
    .then(data=>{
      extras.trendingArtists=Array.isArray(data.artists)?data.artists:[];
      extras.trendingArtistsLoaded=true;
      try{localStorage.setItem("muzeTrendingArtists:v1",JSON.stringify({artists:extras.trendingArtists,saved_at:Date.now()}))}catch(error){}
      renderTrendingArtists();
      return extras.trendingArtists;
    })
    .catch(error=>{console.warn("Unable to load external trending artists",error);extras.trendingArtistsLoaded=false;renderTrendingArtists();return []})
    .finally(()=>{extras.trendingArtistsRequest=null});
  return extras.trendingArtistsRequest;
}
...
function renderArtistResults(options={}){
  const grid=$("#artistResults"),count=$("#artistCount");
  if(!state.artistSearch.trim()){
    artistRenderToken++;
  ...
  let artists=[];
  try{artists=artistDirectoryRows()}catch(error){console.error("Unable to render artist directory",error);if(count)count.textContent="Artists unavailable";if(grid)grid.innerHTML='<div class="empty">Artists could not be loaded.</div>';return}
  if(count)count.textContent=artists.length+" artist"+(artists.length===1?"":"s");
  if(!grid)return;
  artistRenderToken++;
  const token=artistRenderToken;
  if(!options.chunked){grid.innerHTML=artists.map(artistBlock).join("")||'<div class="empty">No artists found.</div>';return}
  grid.innerHTML="";
  if(!artists.length){grid.innerHTML='<div class="empty">No artists found.</div>';return}
  let index=0;
  function renderChunk(){if(token!==artistRenderToken||state.view!=="artists"||!document.body.contains(grid))return;const next=artists.slice(index,index+36).map(artistBlock).join("");if(next)grid.insertAdjacentHTML("beforeend",next);index+=36;if(index<artists.length)requestAnimationFrame(renderChunk)}
  renderChunk();
}
function artistPage(deferResults=false){let letters=["All",..."ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("")];let genres=deferResults&&!artistDirectoryCache?["Popular","Rock","Pop","Alternative Rock","Hip-Hop","Folk Rock","Pop Rock","Metal","Soul"]:artistGenres();let searching=Boolean(state.artistSearch.trim());let artists=!deferResults&&searching?artistDirectoryRows():[];let countLabel=searching?(deferResults?"Loading artists...":`${artists.length} artist${artists.length===1?"":"s"}`):"";let activeLetter=state.artistLetter==="All"?"A-Z Filter":state.artistLetter;return `<div class="artistTopBand"><section class="artistDiscovery"><div><p class="eyebrow">Music culture</p><h2>Artists</h2><p>Discover legendary artists, hidden gems, and community favorites.</p></div><div class="artistSearchWrap"><input id="artistSearchInput" value="${escapeHtml(state.artistSearch)}" oninput="setArtistSearch(this.value)" placeholder="Search artists, albums, genres, moods..."><span id="artistCount">${countLabel}</span></div></section><div class="artistFilterDock"><div class="artistFilterLabel">Genres</div><div class="artistGenreChips">${genres.map(genre=>`<button class="${(state.artistGenre===genre||(genre==="Popular"&&state.artistGenre==="All"))?"active":""}" onclick="setArtistGenre('${escapeJsString(genre)}')">${escapeHtml(genre)}</button>`).join("")}<details class="artistAZMenu"><summary>${activeLetter} <span>v</span></summary><div class="artistAZ">${letters.map(letter=>`<button class="${state.artistLetter===letter?"active":""}" onclick="setArtistLetter('${letter}')">${letter}</button>`).join("")}</div></details></div></div></div><div id="artistTrending">${trendingArtistsMarkup()}</div><div id="artistResults" class="artistGrid">${deferResults||!searching?"":(artists.map(artistBlock).join("")||'<div class="empty">No artists found.</div>')}</div>`}window.setArtistLetter=function(letter){state.artistLetter=letter;syncArtistFilterControls();renderArtistResults({chunked:true})}
function syncArtistFilterControls(){
  document.querySelectorAll(".artistGenreChips button").forEach(button=>{
    const label=(button.textContent||"").trim();
    button.classList.toggle("active",state.artistGenre===label||(label==="Popular"&&state.artistGenre==="All"));
  });
  const summary=document.querySelector(".artistAZMenu summary");
  if(summary)summary.innerHTML=`${state.artistLetter==="All"?"A-Z Filter":escapeHtml(state.artistLetter)} <span>v</span>`;
  document.querySelectorAll(".artistAZ button").forEach(button=>button.classList.toggle("active",(button.textContent||"").trim()===state.artistLetter));
}
window.setArtistGenre=function(genre){state.artistGenre=genre==="Popular"?"All":genre;syncArtistFilterControls();renderArtistResults({chunked:true})}
window.setArtistSearch=function(value){state.artistSearch=value;renderArtistResults({chunked:true});scheduleHomepageCatalogueSearch(value,()=>renderArtistResults({chunked:true}))}
function artistListActiveYears(row){
  const years=(row.albums||[]).map(album=>Number(albumReleaseYear(album)||album.year||0)).filter(year=>year>0).sort((a,b)=>a-b);
  if(!years.length)return "";
  return String(years[0]);
}
function artistListSummary(row){
  const genres=(row.genres||[]).map(value=>String(value||"").toLowerCase());
  const name=String(row.name||"This artist").trim()||"This artist";
  const topTitle=String(row.featured?.[0]?.title||row.hero?.title||"").trim();
  if(genres.some(g=>g.includes("hip-hop")||g.includes("rap")))return `${name} turns rhythm, voice, and perspective into records built for close listening.`;
  if(genres.some(g=>g.includes("metal")))return `${name} channels force, precision, and atmosphere into a catalogue with real weight.`;
  if(genres.some(g=>g.includes("soul")||g.includes("r&b")))return `${name} brings warmth, feeling, and vocal character to songs that linger.`;
  if(genres.some(g=>g.includes("electronic")))return `${name} shapes texture, pulse, and mood into a distinct listening world.`;
  if(genres.some(g=>g.includes("classical")))return `${name} carries a sense of craft, scale, and lasting musical architecture.`;
  if(genres.some(g=>g.includes("folk")))return `${name} keeps the focus on storytelling, melody, and emotional detail.`;
  if(genres.some(g=>g.includes("pop")))return `${name} turns melody and personality into music with broad cultural pull.`;
  if(genres.some(g=>g.includes("alternative")||g.includes("rock")))return `${name} balances edge, melody, and identity across a catalogue made to be revisited.`;
  return topTitle?`${name} is represented in Muze through a focused catalogue led by ${topTitle}.`:`${name} has a focused Muze catalogue shaped by sound, memory, and community taste.`;
}
function artistBlock(row){if(typeof row==="string")row=artistDirectoryRows().find(item=>item.name===row)||{name:row,albums:[],genres:[],hero:{},featured:[],slug:artistSlugForName(row),totalRatings:0,scoreLabel:""};let genres=(row.genres||[]).slice(0,2);let hero=row.hero||row.featured?.[0]||{};let cover=String(hero.cover_url||"").trim();let years=artistListActiveYears(row);let summary=artistListSummary(row);return`<section class="artistCard artistListCard" role="button" tabindex="0" onclick="openArtistPage('${escapeJsString(row.slug)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openArtistPage('${escapeJsString(row.slug)}')}"><div class="artistListMark" aria-hidden="true"><i></i><i></i><i></i></div><div class="artistListCover">${cover?`<img src="${escapeHtml(cover)}" loading="lazy" decoding="async" onerror="this.hidden=true" alt="${escapeHtml(row.name)} image">`:`<span>${escapeHtml(coverText(hero.title?hero:{title:row.name}))}</span>`}</div><div class="artistListCopy"><h3>${escapeHtml(row.name)}</h3>${genres.length?`<div class="artistTags">${genres.map(g=>`<span>${escapeHtml(g)}</span>`).join("")}</div>`:""}<p>${escapeHtml(summary)}</p></div><div class="artistListMeta">${years?`<strong>${escapeHtml(years)}</strong>`:""}</div><span class="artistListArrow" aria-hidden="true">&rsaquo;</span></section>`}
...
window.enhanceArtistPortraitImage=function(image){
  ...
  requestAnimationFrame(()=>{
    ...
    const resolutionRatio=Math.min(image.naturalWidth/width,image.naturalHeight/height);
    image.classList.toggle("needsHdAssist",resolutionRatio<1.35);
  });
};
...
async function loadArtistProfile(slug){
  ...
  if(profile&&!albums.length)albums=state.albums.filter(album=>albumHasArtistCredit(album,cleanSlug));
...
function writeAlbumRoute(album,{replace=false,track="",direct=false}={}){
  ...
  if(replace||sameAlbum)history.replaceState(historyState,"",url);
...
window.openAlbum=async function(id,routeOptions={}){
  ...
  const heroTags=tags.map((tag,index)=>{
    const filterType=index===0?"genre":(a.year&&tag===String(a.year)?"year":"");
    if(!filterType)return `<span>${escapeHtml(tag)}</span>`;
    const label=filterType==="genre"?`View all ${tag} albums`:`View all albums released in ${tag}`;
    return `<span class="albumFilterTag" role="button" tabindex="0" aria-label="${escapeHtml(label)}" onclick="openAlbumOverviewFilter('${filterType}','${escapeJsString(tag)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openAlbumOverviewFilter('${filterType}','${escapeJsString(tag)}')}">${escapeHtml(tag)}</span>`;
  }).join("");
  ...
  const exceptional=Number(albumScore)>=9.6;
  const scoreLabel=exceptional?`<span class="exceptionalStatus"><b>Exceptional</b></span>`:`<span>Rating</span>`;
  const scoreValue=exceptional?`${albumExceptionalFeatherIcon()}${escapeHtml(albumScore)}${albumExceptionalFeatherIcon(true)}`:escapeHtml(albumScore);
  const scoreDetail=`Based on ${Number(total||0).toLocaleString()} rating${Number(total||0)===1?"":"s"}`;
  ...
  const appearanceCard=`<div class="heroSideCard appearances"><h4><span aria-hidden="true">&#9673;</span>Across Muze collections</h4><div class="appearanceMetric"><strong>${appearanceCounts.total.toLocaleString()}</strong><span>appearance${appearanceCounts.total===1?"":"s"}</span></div><p>${appearanceCounts.libraries.toLocaleString()} ${appearanceCounts.libraries===1?"library":"libraries"} &middot; ${appearanceCounts.suggestions.toLocaleString()} suggestion${appearanceCounts.suggestions===1?"":"s"}</p></div>`;
  const soundInfluenceText=structuredOverview.sound_summary||albumCommunityPull(a);
  const soundInfluenceParagraphs=paragraphChunks(soundInfluenceText);
  const soundInfluenceDesktopPreview=soundInfluenceParagraphs.length>1?soundInfluenceParagraphs.slice(0,2).join("\n\n"):compactPreviewText(soundInfluenceText);
  ...
  if(influencePreview){
    const desktopPreview=document.createElement("p");
    desktopPreview.className="influencePreviewDesktop";
    desktopPreview.innerHTML=formatParagraphText(soundInfluenceDesktopPreview);
    influencePreview.insertAdjacentElement("afterend",desktopPreview);
  }
  const editorialSeeMore=$("#albumModal .albumSeeMorePill");
  const albumActionRow=$("#albumModal .linerActions");
  if(editorialSeeMore&&albumActionRow)albumActionRow.insertAdjacentElement("afterend",editorialSeeMore);
  const collectionDataRequests=[];
  if(!extras.profileDirectoryLoaded&&(extras.libraries||[]).length)collectionDataRequests.push(loadProfileDirectory());
  if(!extras.publicListsLoaded)collectionDataRequests.push(loadPublicLists());
  if(!(extras.libraries||[]).length)collectionDataRequests.push(loadLibraries());
  if(collectionDataRequests.length)Promise.allSettled(collectionDataRequests).then(()=>{
    if(albumRef(extras.currentAlbumId)!==albumRef(a.id))return;
    const card=$("#albumModal .heroSideCard.appearances");
    const refreshed=albumPublicAppearanceCounts(a);
    const totalNode=card?.querySelector(".appearanceMetric strong");
    const labelNode=card?.querySelector(".appearanceMetric span");
  ...
  if(heroDescription&&!summary){
    heroDescription.outerHTML=heroReviewMarkup;
    heroDescription=null;
  }else if(heroDescription&&approvedCommunityReview){
    heroDescription.insertAdjacentHTML("afterend",`<p class="albumReviewByline">Review by ${escapeHtml(String(approvedCommunityReview.username||"Listener").replace(/^@/,""))}</p>`);
  }
  if(heroDescription){
  ...
  if(artistHeading)artistHeading.innerHTML=`<button type="button" class="linerArtistLink" onclick="event.stopPropagation();openArtistPage('${escapeJsString(artistSlugForName(a.artist))}')">${escapeHtml(a.artist)}</button> ${albumArtistVerifiedMark()}`;
...
function syncAlbumHeroDescriptionToggle(){
  ...
  const toggle='<button type="button" class="albumHeroDescriptionToggle" aria-expanded="false" onclick="toggleAlbumHeroDescription(this)">See more</button>';
...
window.toggleAlbumHeroDescription=function(button){
  ...
  description.innerHTML=`${description._fullDescriptionHtml} <button type="button" class="albumHeroDescriptionToggle" aria-expanded="true" onclick="toggleAlbumHeroDescription(this)">See less</button>`;
  }else{
...
async function handleMuzeDeepLink(){
  const listSlug=publicListSlugFromPath();
  if(listSlug){openPublicList(listSlug,{replace:true});return}
  const artistSlug=artistSlugFromPath();
  if(artistSlug){navigateToArtist(artistSlug,{replace:true});return}
  const params=new URLSearchParams(location.search);
  const albumId=albumRouteId()||params.get("album");
  const track=params.get("track");
  if(!albumId)return;
  await openAlbum(albumId,{replace:true,direct:true,track});
  if(track){
...
window.rateAlbum=async function(albumId,value){
  ...
  if(db&&!String(albumId).startsWith("seed-")){
    ...
    noteLibraryRecommendationRating();
    await loadData();
    openAlbum(albumId);
  }else{
...
async function loadAllAlbumScoreRows(){
  ...
  for(let from=0;;from+=pageSize){
    ...
    const result=await catalogueDb.from("album_scores").select("*").order("avg_rating",{ascending:false}).range(from,to);
...
async function loadRawAlbumRows(){
  ...
  let result=await catalogueDb.from("albums").select(fields).order("created_at",{ascending:false});
  if(result.error&&/wikipedia_url|source_url|schema cache|column/i.test(result.error.message||"")){
    result=await catalogueDb.from("albums").select("id,title,artist,year,genre,cover_url,spotify_url,summary,spotify_id").order("created_at",{ascending:false});
  }
  if(result.error){
...
async function loadAlbumRuntimeRows(){
  if(!db)return [];
  const catalogueDb=publicDataDb||db;
  const pageSize=1000;
  const rows=[];
  for(let from=0;;from+=pageSize){
    const to=from+pageSize-1;
    const result=await catalogueDb.from("album_metadata").select("album_ref,total_runtime_ms").not("total_runtime_ms","is",null).range(from,to);
    if(result.error)throw result.error;
    const batch=result.data||[];
    rows.push(...batch);
    if(batch.length<pageSize)break;
...
function applyAlbumRuntimeRows(rows=[]){
  if(!Array.isArray(rows)||!rows.length)return false;
  const runtimes=new Map(rows.map(row=>[String(row.album_ref||""),Number(row.total_runtime_ms)||0]).filter(([,value])=>value>0));
  if(!runtimes.size)return false;
  let changed=false;
  state.albums=state.albums.map(album=>{
    const runtime=runtimes.get(albumRef(album.id));
    if(!runtime||Number(album.total_runtime_ms||0)===runtime)return album;
    changed=true;
    return {...album,total_runtime_ms:runtime};
  });
  return changed;
}
let albumRuntimeHydrationRequest=null;
async function hydrateAlbumRuntimes(){
  if(!db)return;
  if(albumRuntimeHydrationRequest)return albumRuntimeHydrationRequest;
  albumRuntimeHydrationRequest=(async()=>{
    try{
      const rows=await loadAlbumRuntimeRows();
      if(applyAlbumRuntimeRows(rows)){
        saveBootAlbums(materializedBootAlbums());
        renderMuzeSearchSuggestions();
      }
    }catch(error){
      console.warn("Muze album runtimes could not be loaded",error?.message||error);
    }finally{
      albumRuntimeHydrationRequest=null;
    }
  })();
  return albumRuntimeHydrationRequest;
}
...
const ALBUM_BOOT_CACHE_KEY="muzeAlbumBootCache:v3";
...
function saveBootAlbums(rows){try{localStorage.setItem(ALBUM_BOOT_CACHE_KEY,JSON.stringify((rows||[]).slice(0,state.homePageSize)))}catch(error){}}
function materializedBootAlbums(){return state.albums.map(album=>{const {_pendingSpotifySave,_justAddedSpotify,...stored}=album;return {...stored,admin_score:score(album),admin_ratings_count:count(album),genre:albumGenreLabel(album)}})}
...
function showPendingSpotifyAlbum(album){
  const pending={...album,id:`pending-spotify-${crypto.randomUUID()}`,avg_rating:0,ratings_count:0,genre:albumGenreLabel(album),_pendingSpotifySave:true,_justAddedSpotify:true};
  state.albums=dedupeAlbumRows([pending,...state.albums]);
  render();
  return pending;
}
function reconcilePendingSpotifyAlbum(pendingId,savedAlbum){
  const saved={...savedAlbum,avg_rating:Number(savedAlbum?.avg_rating||0),ratings_count:Number(savedAlbum?.ratings_count||0),genre:albumGenreLabel(savedAlbum),_justAddedSpotify:true};
...
let homepageRankingsRequest=null;
let homepageRankingsToken=0;
let homepageRankingsAbortController=null;
const homepageFirstPageCache=new Map();
const HOMEPAGE_QUERY_CACHE_MS=60_000;
function normalizeHomepageAlbum(row){
  const effectiveScore=Number(row?.effective_score??row?.admin_score??row?.avg_rating??0);
  const effectiveCount=Number(row?.effective_ratings_count??row?.admin_ratings_count??row?.ratings_count??0);
  return {...row,avg_rating:effectiveScore,ratings_count:effectiveCount,admin_score:effectiveScore,admin_ratings_count:effectiveCount,muze_rank:Number(row?.muze_rank)||null,genre:albumGenreLabel(row)};
}
function mergeUniqueHomepageAlbums(existing=[],incoming=[]){
  if(window.MuzeHomepageLoading)return window.MuzeHomepageLoading.mergeUnique(existing,incoming,album=>String(album?.id||albumIdentityKey(album)||""));
  const seen=new Set();
  return [...existing,...incoming].filter(album=>{
    const key=String(album?.id||albumIdentityKey(album)||"");
    if(!key||seen.has(key))return false;
    seen.add(key);
    return true;
  });
}
function homepageQueryState(){return {searchTerm:String(state.search||"").trim(),genre:String(state.genre||"All"),year:String(state.yearFilter||""),sortMode:String(state.sort||"score"),pageSize:state.homePageSize}}
function homepageQueryKey(query,offset=0){return JSON.stringify([query.searchTerm.toLowerCase(),query.genre.toLowerCase(),query.year,query.sortMode,offset,query.pageSize])}
function homepageReleaseDateSortKey(album){
  const value=String(album?.release_date||"").trim();
  const match=value.match(/^(\d{4})(?:-(\d{1,2}))?(?:-(\d{1,2}))?/);
  const year=Number(match?.[1]||album?.year)||0;
  return year*10000+(Number(match?.[2])||0)*100+(Number(match?.[3])||0);
}
function homepageCompareForMode(mode){
  if(mode==="year")return (left,right)=>homepageReleaseDateSortKey(right)-homepageReleaseDateSortKey(left)||String(left.id).localeCompare(String(right.id));
  if(mode==="ratings")return (left,right)=>count(right)-count(left)||score(right)-score(left)||String(left.id).localeCompare(String(right.id));
  if(mode==="hidden")return (left,right)=>count(left)-count(right)||score(right)-score(left)||String(left.id).localeCompare(String(right.id));
  return (left,right)=>score(right)-score(left)||count(right)-count(left)||String(left.title||"").localeCompare(String(right.title||""))||String(left.id).localeCompare(String(right.id));
}
async function queryHomepageAlbumPage(query,offset=0,{signal,useCache=true}={}){
  ...
  const rows=(payload.albums||[]).map(normalizeHomepageAlbum);
  const page={albums:rows,hasMore:Boolean(payload.hasMore),nextOffset:offset+rows.length,topAlbum:payload.topAlbum?normalizeHomepageAlbum(payload.topAlbum):null,genres:Array.isArray(payload.genres)?payload.genres:[],total:Number(payload.total)||0,query};
  if(offset===0)homepageFirstPageCache.set(cacheKey,{savedAt:Date.now(),page});
  return page;
}
function homepageTimeout(ms,message){
  return new Promise((_,reject)=>setTimeout(()=>reject(Object.assign(new Error(message),{code:"MUZE_HOMEPAGE_TIMEOUT"})),ms));
}
function prefetchHomepageSortPages(){
  const base={searchTerm:"",genre:"All",year:"",pageSize:state.homePageSize};
  ["year","ratings","hidden"].forEach(sortMode=>queryHomepageAlbumPage({...base,sortMode},0,{useCache:true}).catch(()=>{}));
}
async function requestHomepageAlbumPage({reset=false,force=false,query=homepageQueryState()}={}){
  if(homepageRankingsRequest&&!force)return homepageRankingsRequest;
  if(force&&homepageRankingsAbortController){homepageRankingsAbortController.abort();homepageRankingsRequest=null}
  const token=++homepageRankingsToken;
  const offset=reset?0:state.homeOffset;
  const controller=new AbortController();
  homepageRankingsAbortController=controller;
  state.homeLoading=true;
  state.homeLoadState=reset?"loading":"paging";
  state.homeLoadMessage="";
  if(reset){state.albums=[];state.homeOverflow=[];state.homeOffset=0;state.homeHasMore=true;state.homeTotal=null}
  render();
  let slowTimer=0;
  if(reset)slowTimer=setTimeout(()=>{
    if(token!==homepageRankingsToken||!state.homeLoading)return;
    state.homeLoadState="slow";
    state.homeLoadMessage="Still loading Muze rankings…";
    render();
  },4000);
  homepageRankingsRequest=(async()=>{
    try{
      ...
      const defaultGlobalQuery=!query.searchTerm&&query.genre==="All"&&!query.year&&query.sortMode==="score";
      state.albums=visibleRanked.map((album,index)=>defaultGlobalQuery?{...album,muze_rank:index+1}:album);
      const globalTop=page.topAlbum;
      if(globalTop)state.homeTopAlbum=globalTop;
      if(page.genres.length)state.homeGenres=page.genres;
      state.homeTotal=page.total;
      state.homeOffset=page.nextOffset;
      state.homeHasMore=page.hasMore||state.homeOverflow.length>0;
      state.albumCatalogueReady=true;
      state.dataReady=true;
      state.homeLoadState="success";
      state.homeLoadMessage="";
      $("#setupWarning")?.classList.add("hidden");
      applyCachedCovers();
      saveBootAlbums(materializedBootAlbums());
      render();
      if(reset&&!query.searchTerm&&query.genre==="All"&&query.sortMode==="score")setTimeout(prefetchHomepageSortPages,0);
      return true;
    }catch(error){
      if(token!==homepageRankingsToken)return false;
      if(error?.name==="AbortError")return false;
      console.error("[Muze homepage] rankings request failed",error);
      state.homeLoadState="error";
      state.homeLoadMessage="We couldn’t load the rankings.";
      state.dataReady=state.albums.length>0;
      render();
      return false;
    }finally{
      clearTimeout(slowTimer);
      if(token===homepageRankingsToken){state.homeLoading=false;homepageRankingsRequest=null;homepageRankingsAbortController=null;render()}
    }
  })();
  return homepageRankingsRequest;
}
window.retryHomepageRankings=function(){
  state.homeOffset=0;
  state.homeHasMore=true;
  state.homeLoadState="loading";
  state.homeLoadMessage="";
  return requestHomepageAlbumPage({reset:true,force:true});
};
async function loadData(){
  ...
  if(db&&directArtistSlug){
    ...
    if(directArtistResult?.usedDatabase)deepLinkHandled=true;
    else{state.dataReady=false;state.view="rankings";render()}
  }
  if(!db){
  ...
  const bootAlbums=cachedBootAlbums().slice(0,state.homePageSize);
  if(bootAlbums.length){
    state.albums=bootAlbums;
    applyCachedCovers();
    state.dataReady=true;
    state.albumCatalogueReady=true;
    state.homeOffset=bootAlbums.length;
    render();
  }
  const [pageResult,ratingsResult]=await Promise.allSettled([
    requestHomepageAlbumPage({reset:true}),
  ...
  if(pageResult.status==="rejected")console.error("[Muze homepage] startup request failed",pageResult.reason);
...
function closeAlbumPopup(options={}){
  ...
  const returnView=(savedReturnView==="rankings"||window.MuzeRoutes?.VIEW_PATHS?.[savedReturnView])?savedReturnView:"rankings";
...
function goHome(){return navigateToView("rankings",{path:"/"})}
async function navigateToView(view,{replace=false,route=true,path=""}={}){
  const nextPath=path||window.MuzeRoutes?.pathForView(view)||"/";
  if(route){
    const currentPath=window.MuzeRoutes?.normalizePath(location.pathname)||location.pathname;
    const normalizedNext=window.MuzeRoutes?.normalizePath(nextPath)||nextPath;
    const routeState={musica:"view",view};
    if(replace||currentPath===normalizedNext)history.replaceState(routeState,"",nextPath);
    else history.pushState(routeState,"",nextPath);
  }
  if(!$("#albumModal")?.classList.contains("hidden"))closeAlbumPopup({restoreRoute:false});
  state.view=view;
  state.artistProfile=null;state.artistProfileAlbums=[];state.artistEditing=false;state.publicListSlug="";
  syncViewNavigation(state.view);
  render();
  closeNav();
  if(view==="libraries"){
    setTimeout(()=>{
      ...
      loadLibraries()
        .then(()=>{})
        .catch(error=>{
          console.warn("Unable to load libraries",error);
          extras.libraries=localLibraries();
          renderNotifications();
        });
    },700);
  }else if(view==="chat"){
    Promise.all([loadLibraries(),loadChatMessages(),loadUserPresence()])
      .then(()=>{if(state.view===view)render()})
      .catch(error=>{
        console.warn("Unable to load libraries",error);
        extras.libraries=localLibraries();
        renderNotifications();
...
function rememberSiteState(){
  const listSlug=publicListSlugFromPath();
  if(listSlug){history.replaceState({musica:"list",listSlug,direct:true},"",location.href);return}
  const slug=artistSlugFromPath();
  if(slug){history.replaceState({musica:"artist",artistSlug:slug,direct:true},"",location.href);return}
  const routedAlbumId=albumRouteId();
  if(routedAlbumId){history.replaceState({musica:"album",albumId:routedAlbumId,returnView:"rankings",direct:true},"",location.href);return}
  const routedView=window.MuzeRoutes?.viewFromPath(location.pathname)||state.view||"rankings";
  state.view=routedView;
  syncViewNavigation(routedView);
...
window.addEventListener("popstate",async event=>{
  ...
  const listSlug=publicListSlugFromPath();
  if(listSlug){closeAlbumPopup({restoreRoute:false});state.publicListSlug=listSlug;state.view="public-list";render();loadPublicLists().then(()=>{if(state.view==="public-list")render()});return}
  const slug=artistSlugFromPath();
  if(slug){closeAlbumPopup({restoreRoute:false});state.view="artist-profile";loadArtistProfile(slug);return}
  const routedAlbumId=albumRouteId();
  if(routedAlbumId){closeAlbumPopup({restoreRoute:false});await openAlbum(routedAlbumId,{route:false});return}
  const returnView=window.MuzeRoutes?.viewFromPath(location.pathname)||event.state?.view||"rankings";
  await navigateToView(returnView,{route:false});
  if(event.state?.albumId)setTimeout(()=>openAlbum(event.state.albumId,{route:false}),0);
});
let navViewPointerAt=0;
let navViewLastAction="";
function handleNavViewAction(event){
  ...
  event?.stopPropagation?.();
  const now=Date.now();
  const actionKey=`${event?.type||"click"}:${view}`;
  if(now-navViewPointerAt<420&&navViewLastAction.endsWith(`:${view}`))return;
  navViewPointerAt=now;
  navViewLastAction=actionKey;
  const requestedPath=target.dataset.route||window.MuzeRoutes?.pathForView(view)||"/";
  const currentPath=window.MuzeRoutes?.normalizePath(location.pathname)||location.pathname;
  const normalizedRequested=window.MuzeRoutes?.normalizePath(requestedPath)||requestedPath;
  if(state.view===view&&currentPath===normalizedRequested&&!artistSlugFromPath()&&!publicListSlugFromPath()&&!albumRouteId()){closeNav();return}

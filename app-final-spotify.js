...
const extras={tracks:{},trackRatings:{},songScores:{},ratingDetails:{},trackRatingDetails:{},comments:{},commentReplies:{},libraries:[],libraryFollows:[],overviews:{},overviewRequests:{},currentAlbumId:null,spotifyTarget:"musica",previewAudio:null,previewKey:null};
...
function setUsernameSaveBusy(isBusy,label="Save Username"){
  ...
  button.textContent=isBusy?label:"Save Username";
}
async function saveProfileUsername(){
  ...
  if(saved){localStorage.setItem("musicaUsername",saved.username||username);setAuthStatus("Username saved.","success");syncProfileUsernameUi();syncAccountProfileCopy();refreshSavedAvatarDisplay()}
  return saved;
}
...
const metadataArtistGenreFallbacks={
  ...
  "ac dc":"Rock","aerosmith":"Rock","black sabbath":"Metal","bon jovi":"Rock","deep purple":"Rock","def leppard":"Rock","eagles":"Rock","guns n roses":"Rock","iron maiden":"Metal","judas priest":"Metal","led zeppelin":"Rock","motley crue":"Rock","ozzy osbourne":"Metal","slipknot":"Metal","system of a down":"Alternative Metal","van halen":"Rock",
...
function albumGenreLabel(album){
  const manual=String(extras.overviews?.[overviewKey(album)]?.manual_genre||"").trim();
...
function overviewGenerationError(row){return Boolean(row&&row.__generationError)}
function overviewIsGenerating(row){return Boolean(row&&row.__generating&&!overviewGenerationError(row))}
...
function researchPlaceholderOverview(a,row={}){
  ...
  return {intro_summary:`Writing a Muze overview for ${title} by ${artist}.`,sound_summary:"Shaping the sound section around the album itself.",impact_summary:"Finding the album-specific angle before showing the final copy.",legacy_summary:"This overview will refresh once the Muze summary is saved.",quote_headline:"Writing the record into focus."};
}
function normalizeOverviewList(value){
  if(Array.isArray(value))return value.map(item=>typeof item==="string"?item:(item?.name||item?.title||item?.url||"")).map(item=>String(item||"").trim()).filter(Boolean);
...
function albumStructuredOverview(a){
  ...
  const needsResearch=albumNeedsResearch(a,row);
  const isGenerating=overviewIsGenerating(row);
  ...
  const intro=row.intro_summary||sentences[0]||`${a?.title||"This album"} carries the sound and story of ${a?.artist||"its artist"}.`;
  const placeholder=researchPlaceholderOverview(a,row);
  return {
    row,
    isGenerating:Boolean(row.__generating),
    intro_summary:overviewFieldOrPlaceholder(row.intro_summary||intro,placeholder.intro_summary),
...
async function ensureAiAlbumOverview(album,tracks=[],force=false){
  ...
  if(location.protocol==="file:"){
    ...
    extras.overviews[key]={...row,__generating:false,__generationError:"Netlify function unavailable from file://"};
  ...
  try{
    ...
    extras.overviews[key]={...(extras.overviews[key]||{}),__generating:false,__generationError:error.message||String(error)};
  }finally{
...
function albumOverviewHtml(a,{albumId,coverUrl,albumScore,total,customOverview,canEditOverview}){
  ...
  const displaySource=structured.isGenerating?"generating":(structured.row?.fallback_generated?"fallback":(structured.row?.source_summary?"supabase researched cache":(structured.row?.__generationError?"generation error":"local placeholder")));
  ...
  const overviewFocus=escapeHtml(structured.row?.overview_focus||"50% 50%");
...
function setBackCoverFace(album,url){
  ...
  flip.classList.toggle("thrillerBackCrop",albumBackCoverClass(album).trim()==="thrillerBackCrop");
  backFace.innerHTML=`<img src="${escapeHtml(url)}" alt="" loading="lazy" decoding="async" onerror="handleBackCoverImageError(this)"><button class="backCoverInlineZoomButton" type="button" onclick="openBackCoverZoomFromButton(this,event)" aria-label="Zoom back cover"><span class="backCoverMagnifyIcon" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><circle cx="10.5" cy="10.5" r="5.8"></circle><path d="M15 15l5 5"></path></svg></span></button><span class="backCoverLabel">Back cover</span>`;
...
function openBackCoverZoomFromButton(button,event){
  event?.stopPropagation?.();
  const flip=button?.closest?.(".linerCoverFlip");
  const url=flip?.dataset.backCoverUrl||flip?.querySelector?.(".linerCoverBack img")?.getAttribute?.("src")||"";
...
async function loadComments(albumId){
  ...
  if(db){
    ...
    if(!error){
      ...
      if(trackResult.error&&/column|schema cache|avatar_url|user_id|track_name|id/i.test(trackResult.error.message||"")){
...
async function fetchAlbumTracks(album){
  ...
  let tracks=[];
  try{tracks=await requestSpotifyTracks(album,"v18")}catch(e){}
  if(!tracks.length){
    const hadSpotifyId=!!album.spotify_id;
    album=await resolveSpotifyAlbum(album,true);
    try{tracks=await requestSpotifyTracks(album,"v19")}catch(e){}
...
function trackCommentCountsForAlbum(albumId){
  ...
  Object.entries(localTrackComments()).forEach(([storageKey,rows])=>{
    ...
    counts[key]=Math.max(counts[key]||0,(rows||[]).length);
  });
  return counts;
}
...
function updateAlbumSeeMorePlacement(){
  ...
  if(rightSpace>pillWidth+24){
    ...
    pill.style.left=`${Math.max(panelRect.left+24+(pillWidth/2),Math.min(firstAction.left+(firstAction.width/2),panelRect.right-24-(pillWidth/2)))}px`;
    pill.style.top=`${Math.max(firstAction.bottom+18,Math.min(panelRect.bottom-pillHeight-24,firstAction.bottom+24))}px`;
  }
...
function canUseLocalAdminFallback(action){return ["set_album_score","set_rating_count","set_mood_score","set_album_genre","set_loved_track","set_hero_focus","set_overview_focus","set_moment_focus","set_hero_image","set_moment_image"].includes(action)}
...
function albumLibrarySaveCount(album){
  ...
  const countSource=(library,key)=>{
    ...
    if(!items.some(item=>isSameAlbum(item,album)||String(item.id)===String(album.id)))return 0;
    seen.add(key);
    return 1;
  };
...
async function addAlbumToMyLibrary(album){
  ...
  if(items.some(item=>isSameAlbum(item,album)||String(item.id)===String(album.id)))return true;
...
function openLibraryDetails(library){
  ...
  const addAction=isMine?'<button class="libraryDetailAddBtn" onclick="event.stopPropagation();openLibrarySpotifyAdd()">+ Add album</button>':'';
  $("#albumModalContent").innerHTML='<div class="sectionTitle libraryDetailTitle"><div><h2>'+escapeHtml(library.title||"Library")+'</h2><span class="muted">@'+escapeHtml(library.username||"Listener")+' - '+Number(library.album_count||items.length||0)+' albums</span></div><div class="libraryDetailActions">'+addAction+'</div></div><div class="grid libraryFullGrid">'+(items.map(item=>libraryAlbumCard(item,isMine,isMine)).join("")||'<div class="empty">No albums yet.</div>')+'</div>';

...
async function loadData(){if(!db){$("#setupWarning").classList.remove("hidden");state.albums=[...seedAlbums,...localAlbums()];state.ratingMap=localRatings();render();return}let {data:albums,error}=await db.from("album_scores").select("*").order("avg_rating",{ascending:false});if(error){alert(error.message);state.albums=seedAlbums}else state.albums=albums||[];if(!state.albums.length){state.albums=seedAlbums}let {data:ratings}=await db.from("ratings").select("album_id,rating").eq("device_id",state.deviceId);state.ratingMap=Object.fromEntries((ratings||[]).map(r=>[r.album_id,r.rating]));render()}
